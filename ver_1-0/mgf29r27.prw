...
//ฑฑบPrograma  ณ MGF29R27	บAutor  ณ Geronimo Benedito Alves																	บData ณ10/01/18	บฑฑ
//ฑฑฬออออออออออุออออออออออออสอออออออฯอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออสอออออฯออออออออออออนฑฑ
//ฑฑบDesc.		ณ Rotina que mostra na tela os dados da planilha: EEC-Easy Export Control - 29 -EXPORTAวรO - Shippinh Schedule						บฑฑ
...
User Function MGF29R27()
	...
	Aadd(_aDefinePl, "EXPORTAวรO - Shipping Schedule"	)	//01-	_cTitulo	- Titulo da planilha a ser gerada. Aparecerแ na regua de processamento.
	...
	//Se o elemento 2 (nome do campo na view) tem mais de 10 letras ou ้ usado alguma fun็ใo(Sum,Count,max,etc), a ele ้ dado a ele um apelido, indicado    
	//pela clausula "as" que serแ transportado para o elemento 8. Se o apelido nใo for criado, o programa cria-o automaticamente, em tempo de execu็ใo
...
//	Aadd(_aCampoQry, {"EEC_DTEMBA"	,"DATA_BL_TRANSPORTE		as DTBLTRANSP"	,"Data BL Transporte"		,"D",008	,0	,""							,""		,""	 })
	Aadd(_aCampoQry, {"XXPEDIEX01"	,"PEDIDO_EXPORTACAO			as PEDIEXPORT"	,"Pedido Exporta็ใo"		,"C",020	,0	,""							,""		,""	 })
	...
	aAdd(_aParambox,{1,"Neg๓cio"						,Space(tamSx3("EEG_NOME")[1])	,"@!"	,""													,""			,"",115,.F.})	//09											
	...
	EndIF
		...
		_cQuery += U_WhereAnd( !empty(_cCODFILIA ),    " FILIAL_FILTRO IN "                 + _cCODFILIA                               )//OBRIGATORIO (SELEวรO DO COMBO)  CAMPO EMPRESAS(02 posi็๕es)
	_cQuery += U_WhereAnd( !empty(_aRet[2] ),      " NUM_EXP BETWEEN '"            		+ _aRet[1] + "' AND '" + _aRet[2] + "' "   ) //NรO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[3] ),      " ADMINISTRADOR LIKE '%"             + _aRet[3] + "%' "                         )//NรO OBRIGATORIO (USUARIO DIGITA )
	_cQuery += U_WhereAnd( !empty(_aRet[4] ),      " NOME_BUYER LIKE '%"                + _aRet[4] + "%' "                         )//NรO OBRIGATORIO (USUARIO DIGITA )
	_cQuery += U_WhereAnd( !empty(_aRet[5] ),      " NOME_TRADING LIKE '%"              + _aRet[5] + "%' "                         )//NรO OBRIGATORIO (USUARIO DIGITA )
	_cQuery += U_WhereAnd( !empty(_aRet[6] ),      " DESPACHANTE LIKE '%"               + _aRet[6] + "%' "                         )//NรO ษ OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[7] ),      " FAMILA_PRODUTO LIKE '%"            + _aRet[7] + "%' "                         )//NรO ษ OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[8] ),      " PAIS_PORTO_DESTINO_FILTRO LIKE '%" + _aRet[8] + "%' "                         )//NรO ษ OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[9] ),      " NEGOCIO LIKE '%"                   + _aRet[9] + "%' "                         )//NรO ษ OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[10] ),     " PORTO_ORIGEM LIKE '%"              + _aRet[10] + "%' "                        )//NรO ษ OBRIGATORIO
