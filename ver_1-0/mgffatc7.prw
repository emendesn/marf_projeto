...
Liberaçao de bloqueios pendentes de integração Keyconsult
@type function
...
User function MGFFATC7()
	...
	If SZVTMP->(eof())	
		U_MFCONOUT("Não existem pedidos pendentes da keyconsult na tabela SZV!")
		Return
	Else
	...
	Do while SZVTMP->(!eof())	
		...
		U_MFCONOUT("Processando integração keyconsult - " + SZV->ZV_FILIAL + "/" + SZV->ZV_PEDIDO + " - "  + strzero(_nnt,6) + " de " +  strzero(_ntot,6) + "...")
        ...
        If !(SC5->(DBSEEK(SZV->ZV_FILIAL+SZV->ZV_PEDIDO)))
            ...
            _cerro := "Pedido não localizado"
            U_MFCONOUT("Pedido não localizado!")
            _lloop := .T.
            Break
        ...
        Else
            ...
            U_MFCONOUT("Cliente não localizado!")
            _lloop := .T.
            Break
		...
		//Se deu certo já libera o pedido 
        _llibera := .F.
