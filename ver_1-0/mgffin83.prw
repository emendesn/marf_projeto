...
QRY_SE2->(dbGoTop())
IF QRY_SE2->(EOF())
     msgAlert('Nao h� dados para emitir o relatorio !!')
     Return
EndIF
dbSelectArea('SA6')
SA6->(dbSetOrder(1))
...
While QRY_SE2->(!EOF())
	...
	// Inserido este la�o pois o mesmo titulo pode apresentar diversas SE5 - Ajuste em 07/11/18 - Andy e Eric
...
cTexto    := 'AUTORIZAMOS DEBITO NA CONTA CORRENTE NUMERO '+Alltrim(SA6->A6_NUMCON)+IIF(!Empty(SA6->A6_DVCTA),'-'+SA6->A6_DVCTA,'')+;
			 ' DA AGENCIA '+Alltrim(SA6->A6_AGENCIA)+Alltrim(IIF(!Empty(SA6->A6_DVAGE),'-'+SA6->A6_DVAGE,''))+' NA IMPORT�NCIA DE'+;
