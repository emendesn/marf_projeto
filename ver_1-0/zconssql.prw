...
Static Function fCriaMsNew()
    ...
    Conout("[zConsSQL]" + Alltrim(cQuery))
    ...
    For nAtual := 1 To Len(aStruAux)
        ...
        If SX3->(DbSeek(cCampoAtu))
            ...
            If UPPER(cCampoAtu) $ UPPER(cCampoRet)
...
Static Function fConfirm()
    ...
    For nAtual := 1 To Len(aHeadAux)
        ...
        If UPPER(cCampoAtu) $ UPPER(cCampoRet)
            If VALTYPE( aColsNov[nLinAtu][nAtual] ) == "N"
            	cAux += alltrim(STR(aColsNov[nLinAtu][nAtual]))
            Else
