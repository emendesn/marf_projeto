...
DEFINE MSDIALOG oDlg TITLE "Tipo de Valor" FROM 000, 000  TO 320, 650 COLORS 0, 16777215 PIXEL    
	...
	@ 037, 190 SAY  "Total Acr�scimo:"  SIZE 061, 010 OF oDLG                                     COLORS 0, 16777215 PIXEL
	@ 035, 240 MSGET nValA     			SIZE 070, 010 OF oDLG When .F. PICTURE '@E 99,999,999.99' COLORS 0, 16777215 PIXEL

	@ 052, 190 SAY  "Total Decr�scimo:" SIZE 061, 010 OF oDLG                                     COLORS 0, 16777215 PIXEL
	@ 050, 240 MSGET  nValD   			SIZE 070, 010 OF oDLG When .F. PICTURE '@E 99,999,999.99' COLORS 0, 16777215 PIXEL

	@ 067, 190 SAY  "Valor L�quido:"    SIZE 061, 010 OF oDLG                                     COLORS 0, 16777215 PIXEL
	...
	oBrowse:addColumn(TCColumn():new("C�d."     ,{||aLisTV[oBrowse:nAt][02]},"@!" ,,,"LEFT"  ,20,.F.,.F.,,,,,))
...
While ZDS->(!Eof()) .And.        ;
     ...
     IF ZDR->(dbSeek(xFilial('ZDR')+ZDS->ZDS_COD))
        ...
        AAdd(aRec,IIF(ZDR->ZDR_TIPO == 'A','Acr�scimo','Decr�scimo'))
...
IF SE2->E2_VALOR <> SE2->E2_SALDO
     MsgAlert('Acao nao � possivel, motivo : O Titulo sofreu baixa !!')
	 Return
EndIF
IF SE2->E2_MOEDA <> 1
     MsgAlert('Acao nao � possivel, motivo : Moeda do Titulo !!')
	 Return
EndIF
IF Alltrim(SE2->E2_TIPO) $ 'PA#NDF'
     MsgAlert('Acao nao � possivel, motivo : Adiantamento !!')
	 Return
EndIF
...
If !ExistCPO('ZDR',M->ZDS_COD,1)
    ...
    //    MsgAlert('Tipo de Valor j� Cadastrado !!')
	//	lRet := .F.
	//EndIF
//Else                                              
