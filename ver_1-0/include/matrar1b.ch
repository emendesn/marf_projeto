#ifdef SPANISH
	#define STR0001 "Emision del Libro Fiscal de Compras"
	#define STR0002 " desde ["
	#define STR0003 " a "
	#define STR0004 "El objetivo de este programa es imprimir el Libro Fiscal de Compras."
	#define STR0005 "Libros Fiscales "
	#define STR0006 "DENOMINACION"
	#define STR0007 "Nº DOCUMENTO"
	#define STR0008 "IVA"
	#define STR0009 "FECHA"
	#define STR0010 "ESP"
	#define STR0011 "SERIE"
	#define STR0012 "NUMERO"
	#define STR0013 "% IVA"
	#define STR0014 "TOTAL"
	#define STR0015 "GRAVADO"
	#define STR0016 "EXEN/NO GRAV "
	#define STR0017 "IVA"
	#define STR0018 "IVA Percep "
	#define STR0019 " IIBB Percep "
	#define STR0020 "OTROS"
	#define STR0021 "Extranjeros"
	#define STR0022 "Nacionales"
	#define STR0023 "EXCL.NET.GRAV"
	#define STR0024 "TOTALES GENERALES PARA CONTROL GLOBAL DEL IVA"
	#define STR0025 "TOTALIZACION DE TIPO ["
	#define STR0026 "TOTALES"
	#define STR0027 "%IVA"
	#define STR0028 "PROV."
	#define STR0029 "Alicuota"
	#define STR0030 "RESP INSCRITO"
	#define STR0031 "RESP NO INSCRITO"
	#define STR0032 "CF, EXENTOS, NO ALCAN"
	#define STR0033 "MONOTRIBUTISTA"
	#define STR0034 "IMPORTACIONES"
	#define STR0035 "CUADRO, RESUMEN, PARA DDJJ-IVA"
	#define STR0036 "VALOR GRAVADO "
	#define STR0037 "IVA"
	#define STR0038 "PERCEPCION IVA"
	#define STR0039 "CREDITO FISCAL (FA,AT,DB,NC) - SUBTOTAL"
	#define STR0040 "CREDITO FISCAL (CR,ND) - SUBTOTAL "
	#define STR0041 "CREDITO"
	#define STR0042 " FISCAL - TOTAL"
	#define STR0043 "¡Tipo de proveedor no registrado!"
	#define STR0044 "¡Columna de Impuesto no Registrada!"
	#define STR0045 "¡IVA no configurado!"
	#define STR0046 "¡IVAP no configurado!"
	#define STR0047 "¡IB no configurado!"
	#define STR0048 "¡Campos no encontrados! ¡Aplique U_UPDSIGAFIS!"
	#define STR0049 "TD"
	#define STR0050 "VALOR NO GRAVADO "
#else
	#ifdef ENGLISH
		#define STR0001 "Issue of Purchases Tax Record "
		#define STR0002 " from"
		#define STR0003 " to "
		#define STR0004 "The object of this program is to print Acquisition Tax Record."
		#define STR0005 "Tax Records "
		#define STR0006 "DENOMINATION"
		#define STR0007 "NUMBER OF THE DOCUMENT"
		#define STR0008 "IVA"
		#define STR0009 "DATE"
		#define STR0010 "TIO"
		#define STR0011 "SERIES"
		#define STR0012 "NUMBER"
		#define STR0013 "% IVA"
		#define STR0014 "TOTAL"
		#define STR0015 "SAVED"
		#define STR0016 "EXEN/DO NOT SAVE "
		#define STR0017 "IVA"
		#define STR0018 "IVA Percep "
		#define STR0019 " IIBB Percep "
		#define STR0020 "OTHERS"
		#define STR0021 "Foreigners"
		#define STR0022 "Nationals"
		#define STR0023 "EXCL.NET.SAVE"
		#define STR0024 "TOTAL GENERAL FOR IVA GLOBAL CONTROL"
		#define STR0025 "TOTALIZATION OF TYPE"
		#define STR0026 "TOTALS"
		#define STR0027 "%IVA"
		#define STR0028 "SUPPLY."
		#define STR0029 "Rate"
		#define STR0030 "RESP. INSCRIBED"
		#define STR0031 "RESP. NON-SUBSCRIBED"
		#define STR0032 "CF,EXEMPT,NOT REACHED"
		#define STR0033 "MONOTAXPAYER"
		#define STR0034 "IMPORTS"
		#define STR0035 "SUMMARY BOARD FOR DDJJ-IVA"
		#define STR0036 "MONTO GRAVADO "
		#define STR0037 "IVA"
		#define STR0038 "IVA PERCEPCION"
		#define STR0039 "TAX CREDIT (FA,AT,DB,NC) - SUBTOTAL"
		#define STR0040 "TAX CREDIT (CR, ND)  - SUBTOTAL "
		#define STR0041 "CREDIT"
		#define STR0042 " TAX - TOTAL"
		#define STR0043 "Supplier Type not registered!"
		#define STR0044 "Tax column not registered!"
		#define STR0045 "IVA not configured!"
		#define STR0046 "IVAP not configured!"
		#define STR0047 "IB not configured!"
		#define STR0048 "Fields not found! Apply U_UPDSIGAFIS!"
		#define STR0049 "TD"
		#define STR0050 "VALUE NOT SAVED "
	#else
		#define STR0001 If( cPaisLoc $ "ANG|PTG", "Emissão do Livro Fiscal de Compras", "Emissao do Livro Fiscal de Compras" )
		#define STR0002 " de ["
		#define STR0003 " até "
		#define STR0004 If( cPaisLoc $ "ANG|PTG", "O objectivo deste programa é imprimir o Livro Fiscal de Compras.", "O objetivo de este programa e imprimir o Livro Fiscal de Compras." )
		#define STR0005 "Livros Fiscais "
		#define STR0006 If( cPaisLoc $ "ANG|PTG", "DENOMINAÇÃO", "DENOMINACAO" )
		#define STR0007 If( cPaisLoc $ "ANG|PTG", "NR. DOCUMENTO", "NRO DOCUMENTO" )
		#define STR0008 "IVA"
		#define STR0009 "DATA"
		#define STR0010 "ESP"
		#define STR0011 If( cPaisLoc $ "ANG|PTG", "SÉRIE", "SERIE" )
		#define STR0012 If( cPaisLoc $ "ANG|PTG", "NÚMERO", "NUMERO" )
		#define STR0013 "% IVA"
		#define STR0014 "TOTAL"
		#define STR0015 "GRAVADO"
		#define STR0016 If( cPaisLoc $ "ANG|PTG", "ISEN/NÃO GRAV ", "EXEN/NO GRAV " )
		#define STR0017 "IVA"
		#define STR0018 "IVA Percep "
		#define STR0019 " IIBB Percep "
		#define STR0020 If( cPaisLoc $ "ANG|PTG", "OUTROS", "OTROS" )
		#define STR0021 "Estrangeiros"
		#define STR0022 "Nacionais"
		#define STR0023 "EXCL.NET.GRAV"
		#define STR0024 If( cPaisLoc $ "ANG|PTG", "TOTAL GERAIS PARA CONTROLO GLOBAL DO IVA", "TOTAL GERAIS PARA CONTROLE GLOBAL DO IVA" )
		#define STR0025 If( cPaisLoc $ "ANG|PTG", "TOTALIZAÇÃO DO TIPO [", "TOTALIZACAO DO TIPO [" )
		#define STR0026 "TOTAIS"
		#define STR0027 "%IVA"
		#define STR0028 "FORN."
		#define STR0029 If( cPaisLoc $ "ANG|PTG", "Alíquota", "Aliquota" )
		#define STR0030 "RESP. INSCRITO"
		#define STR0031 If( cPaisLoc $ "ANG|PTG", "RESP. NÃO INSCRITO", "RESP. NAO INSCRITO" )
		#define STR0032 If( cPaisLoc $ "ANG|PTG", "CF,ISENTOS,NÃO ALCANÇ", "CF,ISENTOS,NAO ALCAN" )
		#define STR0033 "MONOTRIBUTISTA"
		#define STR0034 If( cPaisLoc $ "ANG|PTG", "IMPORTAÇÕES", "IMPORTACOES" )
		#define STR0035 "QUADRO RESUMO PARA DDJJ-IVA"
		#define STR0036 "MONTO GRAVADO "
		#define STR0037 "IVA"
		#define STR0038 "PERCEPCION IVA"
		#define STR0039 If( cPaisLoc $ "ANG|PTG", "CRÉDITO FISCAL (FA,AT,DB,NC) - SUBTOTAL", "CREDITO FISCAL (FA,AT,DB,NC) - SUBTOTAL" )
		#define STR0040 If( cPaisLoc $ "ANG|PTG", "CRÉDITO FISCAL (CR,ND) - SUBTOTAL ", "CREDITO FISCAL (CR,ND) - SUBTOTAL " )
		#define STR0041 If( cPaisLoc $ "ANG|PTG", "CRÉDITO", "CREDITO" )
		#define STR0042 " FISCAL - TOTAL"
		#define STR0043 If( cPaisLoc $ "ANG|PTG", "Tipo de fornecedor não registado!", "Tipo de fornecedor não cadastrado!" )
		#define STR0044 If( cPaisLoc $ "ANG|PTG", "Coluna de Imposto não registada!", "Coluna de Imposto não Cadastrada!" )
		#define STR0045 "IVA não configurado!"
		#define STR0046 "IVAP não configurado!"
		#define STR0047 "IB não configurado!"
		#define STR0048 "Campos não encontrados! Aplique U_UPDSIGAFIS!"
		#define STR0049 "TD"
		#define STR0050 "VALOR NAO GRAVADO "
	#endif
#endif
