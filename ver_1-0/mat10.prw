...
/*/
USER Function Mat10( cAlias, nReg, nOpcx )
...
While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
	...
	//Incluida validação para os pedidos de compras por item do pedido  (IP/alçada)			
		...
		If MV_PAR15 = 1
			...
			_nLin2   := 00
			cObs51   := ""
			_nTamOBS := INT(LEN(RTRIM(SC7->C7_XCODOBS)) / _nCtPos)
			SZ5->(DBSETORDER(1))
						
		    oReport:Box( 0270 , 0010 , 3020 , nPageWidth )
					   
			FOR _XK := _nCt TO _nTamOBS 
		        _cCod := SUBSTR(SC7->C7_XCODOBS,_nCt,_nCtPos)
		        _nCt  := _nCt + 7 
		        
			    SZ5->(DBSEEK(xFilial("SZ5")+_cCod))  //ZAO
    			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	    		//³ Inicializacao da Observacao do Pedido.                       ³
		    	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			    If !Empty(SZ5->Z5_DESCRIC)   //ZAO
    			   _nObsCom++
				   cVar:="cObs"+StrZero(_nObsCom,2)
				   Eval(MemVarBlock(cVar),SZ5->Z5_DESCRIC)   //ZAO
				   cObs51 := ALLTRIM(SZ5->Z5_DESCRIC)        //ZAO
				   
				   nLin2 := mlcount(cObs51,,175,.T.)

                   For i := 1 to nLin2
		               oReport:SkipLine()
		               oReport:PrintText(memoline(cObs51,175,i,,.F.),,40)
	               Next
		        ...
		        /*If Empty(cObs52)
			       If Len(cObs51) > 65
				      cObs   := cObs51
				      cObs51 := Substr(cObs,1,65)
				      For nXk := 2 To 16
					      cVar  := "cObs"+StrZero(nX,2)
					      &cVar := Substr(cObs,(65*(nXk-1))+1,65)
				      Next nXk
                   EndIf
			    EndIf*/
			NEXT
		...
		Else
//			cObs51:= Substr(cObs51,1,IIf(Len(cObs51)<65,Len(cObs51),65))
//			cObs52:= Substr(cObs52,1,IIf(Len(cObs52)<65,Len(cObs51),65))
//			cObs53:= Substr(cObs53,1,IIf(Len(cObs53)<65,Len(cObs51),65))
//			cObs54:= Substr(cObs54,1,IIf(Len(cObs54)<65,Len(cObs51),65))
//			cObs55:= Substr(cObs55,1,IIf(Len(cObs55)<65,Len(cObs51),65))
//			cObs56:= Substr(cObs56,1,IIf(Len(cObs56)<65,Len(cObs51),65))
//			cObs57:= Substr(cObs57,1,IIf(Len(cObs57)<65,Len(cObs51),65))
//			cObs58:= Substr(cObs58,1,IIf(Len(cObs58)<65,Len(cObs51),65))
		EndIf
