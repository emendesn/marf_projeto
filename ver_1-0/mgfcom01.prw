...
Data................: 23/09/2016
Descricao / Objetivo: WF - Cota��o de compras aos fornecedores
...
//	Local _cMarca  := ""
	...
	//IF MsgYesNo("Deseja enviar o Workflow de Cota��o ?")
		...
		while ! SC8->(eof()) .and. xFilial("SC8")+cCotacao==SC8->C8_FILIAL+SC8->C8_NUM
			...
			if ! Empty(_cEmlFor)
				oProcess := TWFProcess():New( "000001", "Cota��o de Pre�os" )
				...
				oProcess:cSubject	:= "Solicitacao de Cota��o de Pre�os " + _cC8_NUM
...
//				oHtml:ValByName( "totiss"   , cValZero ) 
				...
				IF !"@" $ oProcess:cTO
					...
					AADD(aMsg, " O numero da cotacao � <b>" + _cC8_NUM + "</b> e para participar basta clicar no link logo abaixo :")
					If Len(aProdProc) > 0 .and. File(cArqProdProc)
						AADD(aMsg, "</BR>")
						AADD(aMsg, " Em anexo encontra-se o arquivo em .PDF, com as descricoes dos produtos constantes na Cota��o.")
...
User Function W1881503(AOpcao, oProcess)
	...
	Conout("=============>Inicio Retorno da Cota��o <=============")
	...
	Conout("=============>Termino Retorno da Cota��o <=============")
...
User Function SCHWFCOT()
	...
	Conout("Iniciando o process Workflow Cota��es")
	aWF_Emp := EmpFilWF()
	...
	For _nCot := 1 to len(_aCotas)
		...
		// Cota��o para envio do Workflow
		_cCotacao:= _aCotas[_nCot,3]
		oProcess := TWFProcess():New( "000001", "Cota��o de Pre�os" )
		U_MGFCOM01(2,_cCotacao,oProcess)
	Next _nCot

	Conout("Finalizado processo de Workflow Cota��es.")
Return()
