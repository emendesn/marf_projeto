...
User Function MGF06R27()
	...
	If empty(_aRet[1]) .and. empty(_aRet[7])
		MsgStop("� obrigatorio informar ou a datas de Vencimentos e/ou as Datas dos Pagamentos.")
		Return.F.
	Endif
	...
	_cQuery += U_WhereAnd( !empty(_cCODFILIA ),		" COD_FILIAL IN "			+ _cCODFILIA	                      		 ) //OBRIGATORIO (SELECAO COMBO)
	If !empty(_aRet[2])
		_cQuery += U_WhereAnd( .T. ,   " DT_PAGAMENTO_FILTRO BETWEEN '" + _aRet[1] + "' AND '" + _aRet[2] + "' "	) 
	Endif

	_cQuery += U_WhereAnd( !empty(_aRet[04] ),   " COD_FORNECEDOR  BETWEEN '"      + _aRet[3]  + "' AND '" + _aRet[4]  + "' " ) // NAO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[06] ),   " COD_NATUREZA  BETWEEN '"        + _aRet[5]  + "' AND '" + _aRet[6]  + "' " ) // NAO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[08] ),   " DT_VENCIMENTO_FILTRO BETWEEN '" + _aRet[7]  + "' AND '" + _aRet[8] + "' " ) // NAO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[09] ),   " BANCO_FORNECEDOR = '"           + _aRet[9] + "'  "                        ) // NAO OBRIGATORIO, USUARIO DIGITA, SEM CONSULTA GENERICA
	_cQuery += U_WhereAnd( !empty(_aRet[10] ),   " NUM_BORDERO  BETWEEN '"         + _aRet[10] + "' AND '" + _aRet[11] + "' " ) // NAO OBRIGATORIO
