...
If DbSeek(xFilial("SZF")+SZD->ZD_COD)
	MsgInfo("Nao � possivel excluir este departamento porque possui relacionamento com tabela de preco!","Atencao")
Else
	DbSelectArea("SZE")
	DbSetOrder(1)
...
If DbSeek(xFilial("SZE")+M->ZE_CODDEP+M->ZE_USER)
	MsgInfo("Funcionario j� cadastrado para este departamento!")
	lRet:=.F.
Endif
