...
//ฑฑบPrograma  ณ MGF06R12	บAutor  ณ Geronimo Benedito Alves																	บData ณ  07/03/18  บฑฑ
...
User Function MGF06R12()
	...
	Aadd(_aCampoQry, {"E2_XOBS"		,"OBS_AUDIT"							,"Observa็ใo Produto" 		,"C",2020	,0	,""						,""		,""	})

	Aadd(_aCampoQry, {"CR_DATALIB"	,"DT_LIBERACAO_GRADE1"				,"Data Libera็ใo Aprov 1"			,"D",008	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"CR_DATALIB"	,"DT_LIBERACAO_GRADE2"				,"Data Libera็ใo Aprov 2"			,"D",008	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"CR_DATALIB"	,"DT_LIBERACAO_GRADE3"				,"Data Libera็ใo Aprov 3"			,"D",008	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"CR_DATALIB"	,"DT_LIBERACAO_GRADE4"				,"Data Libera็ใo Aprov 4"			,"D",008	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"CR_DATALIB"	,"DT_LIBERACAO_GRADE5"				,"Data Libera็ใo Aprov 5"			,"D",008	,0	,""						,""		,""	})
	...
	If empty(_aRet[1]) .and. empty(_aRet[4])
		MsgStop("ษ obrigat๓rio informar ou a datas de Vencimentos Real e/ou as Datas dos Pagamentos.")
		Return.F.
	Endif
	...
	If !empty(_aRet[2]) .and. !empty(_aRet[4])
		_cQuery += U_WhereAnd( .T. ,   " ( DT_VENCIMENTO_REAL_F BETWEEN '" + _aRet[1] + "' AND '" + _aRet[2] + "' "  +  ; 
					" OR DT_PAGAMENTO_FILTRO BETWEEN '"   + _aRet[3] + "' AND '" + _aRet[4] + "' )" ) // OBRIGATORIO UM CAMPO OU OUTRO, COM A VALIDAวรO DE 360 DIAS
	...
	_cQuery += U_WhereAnd( !empty(_aRet[6] ),    " COD_FORNECEDOR  BETWEEN '"      + _aRet[5]  + "' AND '" + _aRet[6]  + "' " ) // NรO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[8] ),    " COD_NATUREZA  BETWEEN '"        + _aRet[7]  + "' AND '" + _aRet[8]  + "' " ) // NรO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[10] ),   " DT_VENCIMENTO_FILTRO BETWEEN '" + _aRet[9]  + "' AND '" + _aRet[10] + "' " ) // NรO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[11] ),   " BANCO_FORNECEDOR = '"           + _aRet[11] + "'  "                        ) // NรO OBRIGATORIO, USUARIO DIGITA, SEM CONSULTA GENERICA
	_cQuery += U_WhereAnd( !empty(_aRet[13] ),   " NUM_BORDERO  BETWEEN '"         + _aRet[12] + "' AND '" + _aRet[13] + "' " ) // NรO OBRIGATORIO
