...
Solicitante.........: Cliente
Uso.................: Marfrig
Obs.................: Inclusão manual da data de entrada da Nota Fiscal eletrônica para operações de Busca de Gado - GAP133
=====================================================================================
*/
...
Local cTE    := SuperGetMV("MGF_FIS35T",.T.,"") //Tipos de entrada para operação de busca de Boi
Local nPosTE := aScan(aHeader,{|x| x[2] = "D1_TES"}) //Posição da TES no aCols
Public nQtdDias := SuperGetMV('MGF_FIS35D',.T.,3)

If empty(aCols[1,nPosTE])
	Help( ,, 'MGFFIS35_01',, 'É necessário informar ao menos um Tipo de Entrada (TES)', 1, 0)
	Return
ElseIf !aCols[1,nPosTE]$cTE
	Help( ,, 'MGFFIS35_02',, 'Função não habilidata para o Tipo de Entrada(TES): ' + aCols[1,nPosTE] + '. (MGF_FIS35T)', 1, 0)
	Return
ElseIf !SF1->(FieldPos("F1_ZDTBUSC"))>0
	Help( ,, 'MGFFIS35_03',, 'O Campo F1_ZDTBUSC não existe na base de dados.', 1, 0)
	Return	
EndIf

//Verifica se já foi informada a data de busca anteriormente para trazer para o formulário.
...
If IsInCallStack("MATA103")
	lWhen := .T.
	/*If (IIf(Type("l103Class")!="U",l103Class,.F.)) // Verifica se está classificando a Nota Fiscal
		lWhen := .T.
	Endif*/
Endif
...
lRet := .T.
Else
Help( ,, 'MGFFIS35_04',, 'Informa uma data maior que a data de emissão ou até ' + Alltrim(Str(nQtdDias)) + ' dias após a mesma.', 1, 0)
EndIf
