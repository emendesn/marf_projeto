...
Descricao da alteracao.......: Inclusao de grade de lan�amentos cont�beis. Utiliza as tabelas ZAB, ZAD e ZA0.
==============================================================================================================
*/ 
User Function MGFCOM06()
	...
	oMBrowse:AddLegend( "ZAB_HOMOLO == 'N' .and. Empty(ZAB_VERSAO) ", "YELLOW" , "Em Edi��o" )
	oMBrowse:AddLegend( "ZAB_HOMOLO == 'N' .and. !Empty(ZAB_VERSAO) ", "RED"    , "Vers�o Anterior" )
...
Static Function MenuDef()
	...
	ADD OPTION aRotina TITLE "Gerar Nova Vers�o"    ACTION "U_xMGC6Cop"   OPERATION 9 					   ACCESS 0
...
Static Function ModelDef()
	...
	ElseIf MV_PAR01 == 5 //Lancamento Cont�bil // Alteracao Caroline Cazela 03/12/18 - Contabilidade
		...
		oModel:SetDescription("Grade Cont�bil")
		oModel:SetPrimaryKey({"ZAB_FILIAL","ZAB_CODIGO","ZAB_VERSAO"})
...
Autor...............: Joni Lima
Data................: 29/12/2016
Descricao / Objetivo: Verifica a ultima vers�o e traz a nova
Doc. Origem.........: Contrato - GAP GRADE ERP
Solicitante.........: Cliente
Uso.................: 
Obs.................: Retorna a proxima Vers�o
=====================================================================================
*/
...
Autor...............: Joni Lima
Data................: 28/12/2016
Descricao / Objetivo: Inicializa��o Padrao do TIPO de Grade
Doc. Origem.........: Contrato - GAP GRADE ERP
Solicitante.........: Cliente
Uso.................: 
	...
	If !LCOPY
		...
		If ZAB->(DbSeek(xFilial('ZAB') + xValue ))
			...
			oMdlZAB:GetModel():SetErrorMessage(oMdlZAB:GetId(),cFld,oMdlZAB:GetModel():GetId(),cFld,cFld,;
				"J� Existe esse Codigo (Tipo +  Centro de Custo)", "Para realizar manutencao no registro sera necessario realizar uma revisao da Grade" + oMdlZAB:GetValue('ZAB_TIPO') + xValue )
		EndIf
	EndIf
	RestArea(aAreaZAB)
	RestArea(aArea)
...
User Function xMGC6Alt()
	...
	Else
		Alert('S� � Permitida a alteracao das Grades que nao estao homologadas')
	EndIf
Return
...
User Function xMGC6Exc()
	...
	Else
		Alert('S� � Possivel realizar a Exclusao de Grades que nao estejam homologadas')
	EndIf
...
Autor...............: Joni Lima
Data................: 28/12/2016
Descricao / Objetivo: Realizar Geracao de uma nova vers�o para uma Grade
Doc. Origem.........: Contrato - GAP GRADE ERP
Solicitante.........: Cliente
Uso.................: 
Obs.................: Gera uma nova vers�o da Grade
=====================================================================================
*/
User Function xMGC6Cop()
	...
	If ZAB->ZAB_HOMOLO == 'S'
		...
		Else
			Alert('J� existe uma grade em edicao para esse codigo: ' + cCod)
		EndIf
	Else
		Alert('S� � possivel gerar uma nova vers�o de grade da ultima vers�o homologada')
	EndIf
	LCOPY := .F.
...
User Function xMGC6CC()
	...
	If !Empty( ZAB->ZAB_VERSAO )
		Help( ,, 'Grade - C�pia',, 'Selecione Grade Em Edi��o para geracao de c�pias', 1, 0 )
		RestArea( aArea )
		Return
	EndIf
	...
	*/
	...
	If !ParamBox(aPergs ,"Grade - Parametros de C�pia",aRet)
		Help( ,, 'Grade - C�pia',, 'Processamento Cancelado', 1, 0 )
		RestArea( aArea )
		Return
	EndIf
	...
	If !Empty( (cNextAlias)->ZAB_RECNO )
		...
		If MsgYesNo("Existe(m) "+AllTrim(Str(nCnt))+" grade(s) em edicao no intervalo selecionado"+CRLF+;
					"Deseja sobrepor as informacoes existentes?","C�pia de Grade")
...
User Function xM06Homl()
	...
	*/
	...
	If !ParamBox(aPergs ,"Grade - Parametros de Homologa��o",aRet)
		Help( ,, 'Grade - Homologa��o',, 'Processamento Cancelado', 1, 0 )
		RestArea( aArea )
		Return
	EndIf
	...
	Else
		Help( ,, 'Grade - Homologa��o',, 'Nao  existem registros para os parametros informados!', 1, 0 )
	EndIf
...
Autor...............: Joni Lima
Data................: 11/04/2016
Descricao / Objetivo: Realiza a Homologa��o da grade
=====================================================================================
*/
...
Obs.................: Utilizado nas validacoes de Inclusao de contabiliza��es txt e automaticas(Manuais)
=====================================================================================
*/
