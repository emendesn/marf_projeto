...
Autor....:              Anderson Reis   
Data.....:              21/10/2019 
Descricao / Objetivo:   Integracao PROTHEUS - MERCADO ELETR�NICO
...
If FunName() == 'MATA121' .And. (Paramixb[1] == 4 .OR. Paramixb[1] == 5 .OR. Paramixb[1] == 9)  
	...
	If  ! Empty(SC7->C7_ZPEDME)
		If Alltrim(cusername) $ alltrim(cusers) .and. FunName() == 'MATA121' .And. Paramixb[1] == 4 
			...
			return .t.
		Else
			MsgAlert(FunName() +'Este Pedido foi integrado via Mercado Eletronico (ME), nao � possivel Alterar/Excluir/Copiar')
			lExecuta := .F.
		endif
	EndIf
EndIf
