...
User Function MGF78R04()
	...
	Aadd(_aDefinePl, "Frete Embarcador - Tabela de Frete V�nculo"	)	//01-	_cTitulo	- Titulo da planilha a ser gerada. Aparecera na regua de processamento.
	Aadd(_aDefinePl, "Tabela de Frete V�nculo"					)	//02-	_cArqName  - Nome da planilha Excel a ser criada
	Aadd(_aDefinePl, {"Tabela de Frete V�nculo"}					)	//03-	_cNomAbAna - Titulo(s) da(s) aba(s) na planilha excel
	Aadd(_aDefinePl, {"Tabela de Frete V�nculo"}					)	//04-	_cNomTTAna - Titulo(s) da(s) tabela(s) na planilha excel
	...
	Aadd(_aCampoQry, {"A1_FILIAL"	,"COD_FILIAL		as CAMP01"	,"Codigo Filial"					,"C"	,006,0	,"" 					,"" ,"" })
	...
	Aadd(_aCampoQry, {"GVA_DTNEG"	,"DATA_NEGOCIACAO	as CAMP09"	,"Data Negociacao"					,"D"	,008,0	,"" 					,"" ,"" })
	...
	Aadd(_aCampoQry, {"GVA_OBS"		,"OBSERVACAO		as CAMP14"	,"Observacao"						,"C"	,040,0	,"" 					,"" ,"" })
	...
	Aadd(_aCampoQry, {"GVA_DTAPR"	,"DATA_APROV		as CAMP26"	,"Data Aprovacao"					,"D"	,008,0	,"" 					,"" ,"" })
	...
	aAdd(_aParambox,{3,"Situacao"			,1			,{"Liberada","Em Negociacao","Nao Informar"}	,050,""	,.F.})

	If ! U_ParameRe(_aParambox, _bParameRe, @_aRet) ; Return ; Endif

	If Empty(_aRet[1])
		MsgStop("O Tipo de Operacao � obrigatorio informar.")
		Return .F.
	EndIf
	...
	_cQuery += iif(!empty(_cSitua ), " AND SITUACAO  = 			'" + _cSitua + "' "	, " "			     )	+ CRLF	//Tipo de Operacao
