...
//±±ºPrograma  ³ MGF29R33	ºAutor  ³ Geronimo Benedito Alves																	ºData ³08/01/18	º±±
...
//±±ºDesc.		³ Rotina que mostra na tela os dados da planilha: EEC-Easy Export Control - 29 -EXPORTAÇÃO - Controle AWB x ETA						º±±
...
User Function MGF29R33()
	...
	Aadd(_aDefinePl, "EXPORTAÇÃO - Performance NF x BL"	)		// 01-	_cTitulo	- Titulo da planilha a ser gerada. Aparecerá na regua de processamento.
	...
	Aadd(_aCampoQry, {"A1_NOME"		,"BUYER"									,"Buyer"			,"C",040	,0	,""		,""		,""	})
 	Aadd(_aCampoQry, {"YA_DESCR"	,"PAIS_PORTO_DESTINO	as PAIS_DEST"		,"País Destino"		,"C",025	,0	,""		,""		,""	})
	...
	_cQuery += U_WhereAnd( !empty(_aRet[2] ),      " DATA_NF_FILTRO BETWEEN '"   + _aRet[1]  + "' AND '" + _aRet[2] + "'  " ) // OBRIGATORIO, COM A VALIDAÇÃO DE 35 DIAS
	_cQuery += U_WhereAnd( !empty(_aRet[3] ),      " ADMINISTRADOR LIKE '%"             + _aRet[3] + "%' "                         )//NÃO OBRIGATORIO (USUARIO DIGITA )
	_cQuery += U_WhereAnd( !empty(_aRet[4] ),      " FAMILA_PRODUTO LIKE '%"            + _aRet[4] + "%' "                         )//NÃO É OBRIGATORIO
