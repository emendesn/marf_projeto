...
static function defRelat()
	...
	local cTitRel	:= "Relatorio de An�lise de Mercado Interno"
	local cDescRel	:= "Relatorio de An�lise de Mercado Interno"
	...
	oReport:lOnPageBreak := .T.					//Cabeçalho das seções impressas após a quebra de página
	oReport:setTotalInLine(.F.)					// Define que a impressão dos totalizadores será em linha. Falso imprime em linha.
	//oReport:nFontBody := 9
	//oReport:cFontBody := "Arial"
	//oReport:SetLineHeight(55)

	oSecZAV := TRSection():New(oReport, cNomeRel,,,,, /*uTotalText*/,.T.)
	oSecZAV:SetHeaderSection(.T.)				// Define que imprime cabeçalho das células na quebra de seção
	oSecZAV:SetHeaderBreak(.T.)
	oSecZAV:lOnPageBreak := .T.				// Cabeçalho das seções impressas após a quebra de página
	oSecZAV:setTotalInLine(.F.)				// Define que a impressão dos totalizadores será em linha.

	oSecZAW := TRSection():New(oSecZAV, "Ocorr�ncias",,,,,,.T.)
	oSecZAW:SetHeaderSection(.T.)				// Define que imprime cabeçalho das células na quebra de seção
	oSecZAW:SetHeaderBreak(.T.)
	oSecZAW:lOnPageBreak := .T.				// Cabeçalho das seções impressas após a quebra de página

	oSecZAX := TRSection():New(oSecZAW, "Resolu��es",,,,,,.T.)
	oSecZAX:SetHeaderSection(.T.)				// Define que imprime cabeçalho das células na quebra de seção
	oSecZAX:SetHeaderBreak(.T.)
	oSecZAX:lOnPageBreak := .T.				// Cabeçalho das seções impressas após a quebra de página
...
//	TRCell():New(oSecZAX, "ZAX_SERIE"		, "QRYZAV"	, "Serie"			, 					, , , , "LEFT"				, 					, "LEFT"					,, 1 /*nColSpace*/)
	...
	//TRCell():New(oSecZAX, "ZAX_RESOLU"		, "QRYZAV"	, "Resolu��o"		, 					, , , , "LEFT" /*cAlign*/	, .T./*lLineBreak*/	, "LEFT" /*cHeaderAlign*/	,, 1 /*nColSpace*/)
...
Função estática, chamada pela defRelat, que seleciona os dados para
