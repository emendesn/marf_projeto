...
//ฑฑบPrograma  ณ MGF29R16	บAutor  ณGeronimo Benedito Alves																	บData ณ22/05/18    บฑฑ
//ฑฑฬออออออออออุออออออออออออสอออออออฯอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออสอออออฯออออออออออออนฑฑ
//ฑฑบDesc.		ณ Rotina que mostra na tela os dados da planilha: EEC-Easy Export Control - 29 -EXPORTAวรO - Seguro Internacional				   บฑฑ
...
User Function MGF29R16()
	...
	Aadd(_aDefinePl, "Exporta็ใo - Seguro Internacional")	//01-	_cTitulo	- Titulo da planilha a ser gerada. Aparecerแ na regua de processamento.
	...
	Aadd(_aCampoQry, {"EEC_ZEXP"	,"NUMERO_EXP"								,"N๚mero Exporta็ใo"	,"C",013	,0	,""		,""		,""	})
	Aadd(_aCampoQry, {"EEC_ZANOEX"	,"ANO"										,"Ano Exporta็ใo"		,"C",002	,0	,""		,""		,""	})
	...
	aAdd(_aParambox,{1,"C๓d. Buyer Inicial"			,Space(tamSx3("A1_COD")[1])		,""		,""													,"CLI"	,"",050,.F.}) 
	aAdd(_aParambox,{1,"C๓d. Buyer Final"			,Space(tamSx3("A1_COD")[1])		,""		,"U_VLFIMMAI(MV_PAR02, MV_PAR03,'Cod. Buyer')"		,"CLI"	,"",050,.F.})	
	aAdd(_aParambox,{1,"Data Emissใo NF Inicial"	,Ctod("")						,""		,""													,""		,"",050,.F.}) 
	aAdd(_aParambox,{1,"Data Emissใo NF Final"		,Ctod("")						,""		,"U_VLFIMMAI(MV_PAR04, MV_PAR05,'Data Emissใo')"	,""		,"",050,.F.})	
	...
	cQryPaisde	:= "SELECT ' ' as YA_NOIDIOM, '  Nใo Informado' as YA_DESCR, ' ' as YA_CODGI FROM DUAL UNION ALL " +CRLF
	...
	aCpoPaisde	:=	{	{ "YA_NOIDIOM"	,"Paํs porto Destino (no idioma)"		,TamSx3("YA_NOIDIOM")[1] + 80	}	,;
						{ "YA_DESCR"	,"Paํs porto Destino"		,TamSx3("YA_DESCR")[1] + 80	}	,;
						{ "YA_CODGI"	,U_X3Titulo("YA_CODGI")	,TamSx3("YA_DESCR")[1] 		}	 } 
	cTituPaisd	:= "Paํses dos portos de Destinos a serem listados: "
	...
	cQryVia	:= " SELECT '" +SPACE(TamSx3("YQ_DESCR")[1])+ "' as YQ_DESCR, 'Nใo Informado' as YQ_COD_DI, '  ' as YQ_VIA FROM DUAL UNION ALL " +CRLF
	...
	aCpoVia	:=	{	{ "YQ_DESCR"		,"Via de Transporte"	,TamSx3("YQ_DESCR")[1] + 40	}  ,; 
					{ "YQ_COD_DI"		,"C๓digo"				,TamSx3("YQ_COD_DI")[1] + 30 }  ,; 
	...
	_cQuery += U_WhereAnd( !empty(_aRet[1] ),     " ADMINISTRADOR LIKE '%"            + _aRet[1] + "%' "                         ) //NรO ษ OBRIGATORIO, USUมRIO DIGITA
	_cQuery += U_WhereAnd( !empty(_aRet[3] ),     " CODIGO_BUYER BETWEEN '"           + _aRet[2] + "' AND '" + _aRet[3] + "' "   ) //NรO OBRIGATORIO --SELEวรO POR RANGE  **NOVO FILTRO**
	...
	_cQuery += U_WhereAnd( !empty(_aRet[5] ),     " NF_FILTRO BETWEEN '" + _aRet[4]   + "' AND '" + _aRet[5] + "' "              ) //NรO OBRIGATORIO, PODE VIR EM BRANCO --SEM TRAVA
	_cQuery += U_WhereAnd( !empty(_aRet[7] ),     " DATA_ESTUFAGEM_FILTRO BETWEEN '"  + _aRet[6] + "' AND '" + _aRet[7] + "' "   ) //NรO OBRIGATORIO, PODE VIR EM BRANCO --SEM TRAVA
	_cQuery += U_WhereAnd( !empty(_aRet[9] ),     " DATA_BL_FILTRO BETWEEN '"         + _aRet[8] + "' AND '" + _aRet[9] + "' "   ) //NรO OBRIGATORIO, PODE VIR EM BRANCO --SEM TRAVA
	If Empty( _cVia )
		_cQuery += ""
	ElseIF AT("' '", _cVia ) <> 0
