...
User Function MGFCOMBG()
    ...
    // Verifica se o campo chave cont�m caracteres diferente de n�meros
    lRetChv := .t.
    ...
    Next 
    IF !lRetcHV
        MsgAlert("A Chave da NFe deve conter somente n�meros.","MGFCOMBG")
        lRet := .F.
        Return lRet
    EndIf
...
/*Mes Emissao*/
    ...
    IF _cMesEmis <> _chMes 
        _cMsg += "M�s Divergente Chave x Cabecalho da Nota"+CRLF
        lRet := .F.
    ENDIF
        ...
        IF _cCnPjFor <> _chCnpj                
            If ! ZG4->(DbSeek(xFilial("ZG4")+_chCnpj))            
                If _cTipo="F"
                    ...
                    lRet := .F.
                ElseIf _cTipo="J"
                    _cMsg += "Pessoa Jur�dica, CNPJ Divergente Chave x Cabecalho da Nota"+CRLF
                    lRet := .F.
                EndIf
            EndIf
        EndIf
...
/*Digito Chave*/
    ...
    If _cDigChNf <> Val(_chDigVer)
        _cMsg += "Digito Verificador - Chave de acesso informada � inv�lida"+CRLF
        lRet := .F.
    EndIf
...
For nI := 1 TO Len(cSerie)
    ...
    ElseIf SubStr(cSerie,nI,1) $ "-*+!@#$%�&*()=/<>;.,"
        MsgAlert("Existem caracteres inv�lidos no campo serie. Informe a serie da nota novamente.","MGFVALSR")
        cTexto := SPACE(03)
    EndIF
Next nI
...
For nI := 1 TO Len(cnfiscal)
    ...
    ElseIf SubStr(cnfiscal,nI,1) $ "-*+!@#$%�&*()=/<>;.,"
        MsgAlert("Existem caracteres inv�lidos no campo da nota fiscal. Informe o numero da Nota novamente.","MGFVALNF")
        cTexto := SPACE(09)
    EndIF
Next nI
