...
/*
	Private cPatSrv	:= GetMv("MGF_FIN36A",,"\MGF\CRE25\") 		 // Path de grava็ใo de Arquivos (Server)
	Private cPatLoc	:= GetMv("MGF_FIN36B",,"C:\PROTHEUS\CRE25\") // Path de grava็ใo de Arquivos (Local)
...
*/
	...
	*/
	...
	//	"Valor Devolu็ใo"
	//aColunas :={{"Valor Devolu็ใo"	,{ || nValDev } ,"N",cPicVal,2/* */,aTamVal[1],aTamVal[2]}}
	//	"Valor Saldo"
	// E1_SALDO
	//	"% Pago"
...
*/
	...
	//descri็ใo do browse
	oBrowse:SetDescription("Acompanhamento Cobran็a")
...
Static Function ModelDef()
	...
	// Busca dados do usuแrio para saber qtos digitos usa no ANO.
	PswOrder(2)
	If PswSeek( _cNomUsr, .T. )
		aDadosUsu := PswRet() // Retorna vetor com informa็๕es do usuแrio
...
ฑฑบPrograma  ณMGFFIN36  บAutor  ณMicrosiga           บ Data ณ  11/01/16   บฑฑ
...
Static Function FIN36Monta()
	...
	DEFINE MSDIALOG oDlg TITLE "Acompanhamento Cobran็a" FROM 000, 000  TO 500, 1100 COLORS 0, 16777215 PIXEL
...
Static Function fListBox1()
	...
	Else
		...
		While (_cAlias1)->(!EOF())
			...
			Else
				//-----------------------------------
				//Se nใo houver no Pedido, busca cliente
				//-----------------------------------
			...
			Transform(nValDev,cPicE1_Val)				,;	//12 Valor devolu็ใo		??da onde pegar
			...
			cZZ9Posicao									,;  //31 Posicao de cobran็a
...
Static Function MGFIN36AA(xOpc)
	...
	If Empty(cFilSE1)
		APMsgStop("Para usar a exporta็ใo para Excel, ้ necessแrio realizar um Filtro antes no Browse."+CRLF+;
		"Utilize o botใo 'Filtrar' localizado no canto direito de cima do Browse.")
		Return()
	Endif
	...
	"Para uma quantidade grande de registros a exporta็ใo pode ser demorada."+CRLF+;
	"Deseja continuar ?")
		If !Empty( cFilSE1 )
			SE1->( DBClearFilter() )
		EndIf
		...
		If xOpc == "1"
			...
			Else
				...
				If xFil <> SE1->E1_FILIAL
					...
					Else
						Alert("Nใo existe destinatแrio cadastrado nessa sele็ใo!")
					EndIf
...
//		cQuery += " WHERE ZZB.ZZB_FILIAL  = '" 	+ xFilial("ZZB") 	+ "' "   // NAO TRATAR FILIAL CONFORME CLAUDIO 16/04/18
		...
		aadd(aLinha, GetAdvFVal("ZZ9","ZZ9_DESPOS",xFilial("ZZ9")+(cAliasB)->ZZB_CODPOS,1,"")	) //31 Posicao de cobran็a
	...
	If _lLinha
		...
		If xOpc == "2"
			...
			if __CopyFile(cArq, cLocArq + cArq)
				MsgInfo("Relat๓rio gerado em: " + cLocArq + cArq)
				oExcelApp:WorkBooks:Open(cLocArq + cArq)
				oExcelApp:SetVisible(.T.)
			else
				MsgInfo("Arquivo nใo copiado para o Diretorio " + cLocArq + cArq)
			endif
		Else
			//------------------------
			//Envia Email
...
Static Function EnvEmail(xTo, xArq)
	...
	Local cEmail	:= Alltrim(SUPERGETMV("MGF_EMAIL"  ,,"teste.any@marfrig.com.br")) //SUPERGETMV("MGF_EMAIL"		,,"teste.any@marfrig.com.br"/*""*/)  									//email que serแ incluido no FROM
	Local cMsg 		:= ""
	Local nSendPort := 0
	...
	oMessage:cBody 		:= "Segue arquivo para acompanhamento de cobran็a."
...
Autor...............: Barbieri
Data................: 07/02/2017
Descri็ใo / Objetivo: Gatilho para campo E1_ZCODSEG
...
/* Fun็ใo que retorna as intera็๕es de cobran็a*/
...
/* retirado por Paulo Fernandes devido ao join nใo estar atendendo a pesquisa 21/05/2018
...
Local cVar     := Nil
Local oDlg     := Nil
Local cTitulo  := "Consulta acompanhamentos de cobran็a"
...
Private cCadastro := "Acompanhamento de cobran็a"
Private lChk   := .F.
Private oLbx   := Nil
...
//+-----------------------------------------------+
If Len( aVetor1 ) == 0
   Aviso( cTitulo, "Nao hแ intera็๕es", {"Ok"} )
Else


DEFINE MSDIALOG oDlg TITLE cTitulo FROM 0,0 TO 240,500 PIXEL
dBselectArea('ZZB')
@ 10,10 LISTBOX oLbx VAR cVar FIELDS HEADER "Tํtulo", "Parcela", "Tipo", "Data intera็ใo", "Hora intera็ใo", "Recno" SIZE 230,095 OF oDlg PIXEL ;
