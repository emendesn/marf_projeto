...
User Function MGFFIN43()
	...
	If nMgBco==1
		Private cArqCfg	:= GetMv("MGF_FIN43B",,"\EDI\ITAU\FIDC\CFG\FIDC.REM")	// Arquivo de Configura��o FIDC
		Private cPatLoc	:= GetMv("MGF_FIN43D",,"C:\ITAU\FIDC\REM\")				// Path de gravacao de Arquivos
	Else
		Private cArqCfg	:= GetMv("MGF_FIN44B",,"\EDI\BRA\FIDC\CFG\FIDC.REM")	// Arquivo de Configura��o FIDC
	...
	If nMgBco==1
		Private cArqCfgRec	:= GetMv("MGF_FIN43K",,"\EDI\ITAU\FIDC\CFG\RECOMPRA.REM")	// Arquivo de Configura��o Recompra
		Private nLinGrid	:= GetMv("MGF_FIN43L",,30000)	// Numero maximo de linhas no grid, padrao = 990
	Else
		Private cArqCfgRec	:= GetMv("MGF_FIN44K",,"\EDI\BRA\FIDC\CFG\RECOMPRA.REM")	// Arquivo de Configura��o Recompra
...
Static Function ViewDef()
	...
	oView:AddUserButton(�'Busca de Titulo'��,�'CLIPS',�{|oView|�U_MGFFIN71()}�)
...
Static Function fValidTipo(oModel)
	...
	If !Empty(ZA7->ZA7_DATA) .And. ( oModel:GetOperation() == MODEL_OPERATION_UPDATE .Or. oModel:GetOperation() == MODEL_OPERATION_DELETE )
		lRet		:= .F.
		Help( ,, 'ARQGERADO',, 'Arquivo remessa j� foi gerado.'+CRLF+'Nao � permitido '+IIF(oModel:GetOperation() == MODEL_OPERATION_UPDATE,"alterar","excluir")+".", 1, 0 )
...
Static Function fbPreLineGrid(oModelGrid,nLinha,cAcao,cCampo,xValor)
	...
	If oModelGrid:GetOperation() == MODEL_OPERATION_UPDATE
		If cAcao == "DELETE"
			Help( ,,"Atencao", ,"Nao � permitido exclusao de linhas!",1,0)
			lRet := .F.
		EndIf
	EndIf
...
SM0_CODFIL  // Codigo da filial contendo todos os n�veis (Emp/UN/Fil)
SM0_EMPRESA // Codigo da empresa
SM0_UNIDNEG // Codigo da unidade de neg�cio
	...
	If !IsBlind()
		...
		If PswSeek( __cUserID, .T. )
			...
			For nInc := 1 To Len( aSM0 )
				...
				//Na funcao FWLoadSM0(), ela retorna na posicao [SM0_USEROK] se esta filial � valida para o user  
...
Else
	Help("  ",1,"ADMFILTOP",,"Funcao dispon�vel apenas para ambientes TopConnect",1,0)
EndIf
...
DEFAULT cTitulo			:= OemToAnsi( "Escolha Padr�es" )
DEFAULT aOpcoes			:= {}
DEFAULT cOpcoes			:= ""
DEFAULT l1Elem			:= .F.
...
Begin Sequence
	...
	*/
	...
	IF !( lComboBox )
		...
		Else
			...
			����������������������������������������������������������������
			*/
			MsgInfo( OemToAnsi( "Nao existem dados para consulta" ) , IF( Empty( cTitulo ) , OemToAnsi( "Escolha Padr�es" ) , cTitulo ) )
			Break
		EndIF	
	Else
	...
	*/
	...
	DEFINE MSDIALOG oDlg TITLE cTitulo FROM aAdvSize[7],0 TO aAdvSize[6]+50,aAdvSize[5]+nSize OF GetWndDefault() PIXEL //"Escolha Padr�es"
	...
	If !lVisual
		...
		*/
		...
		����������������������������������������������������������������
		*/
		@ aGrpCoords[1]+10,(aGrpCoords[2]+75+4)			GROUP oGrpRet	TO aGrpCoords[3]+15+5,152.50	OF oDlg LABEL OemtoAnsi("M�x. Elem. p/ Selecao")   PIXEL
...
/*
�����������������������������������������������������������������������Ŀ
�Funcao    �CaPexTroca	    �Autor�Marinaldo de Jesus � Data �11/09/2003�
�����������������������������������������������������������������������Ĵ
�Descricao �Efetua a Troca da Selecao no ListBox da AdmOpcoes()   		�
�����������������������������������������������������������������������Ĵ
...
/*
�����������������������������������������������������������������������Ŀ
�Funcao    �fOpcPesqF3		�Autor�Marinaldo de Jesus � Data �11/11/2004�
�����������������������������������������������������������������������Ĵ
�Descricao �Efetua Pesquisa Via Tecla F3                         		�
�����������������������������������������������������������������������Ĵ
�Sintaxe   �<Vide Parametros Formais>									�
�����������������������������������������������������������������������Ĵ
�Uso       �AdmOpcoes()                                                 �
...
/*
�����������������������������������������������������������������������Ŀ
�Funcao    �AdmOpcOrd	    �Autor�Marinaldo de Jesus � Data �11/09/2003�
�����������������������������������������������������������������������Ĵ
�Descricao �Ordenar as Opcoes em AdmOpcoes                        		�
�����������������������������������������������������������������������Ĵ
�Sintaxe   �<Vide Parametros Formais>									�
�����������������������������������������������������������������������Ĵ
�Uso       �AdmOpcoes()                                                 �
...
DEFINE MSDIALOG oDlg TITLE OemToAnsi(cTitulo) From aAdvSize[7],0 TO aAdvSize[6],aAdvSize[5] OF GetWndDefault() PIXEL			
	...
	@ aObjSize[1,1] , aObjSize[1,2] GROUP oGroup TO aObjSize[1,3],aObjSize[1,4] LABEL OemToAnsi("Ordena��o") OF oDlg PIXEL
	oGroup:oFont:= oFont

	@ ( aObjSize[1,1] + 010 ) , ( aObjSize[1,2]+005 )	SAY OemToAnsi("Efetuar a Ordena��o por:")	SIZE 300,10 OF oDlg PIXEL FONT oFont
	@ ( aObjSize[1,1] + 010 ) , ( aObjSize[1,2]+100 )	RADIO oRadio VAR nOpcRad	ITEMS 	OemToAnsi("codigo"),;
																	 						OemToAnsi("descricao"),;
																	 						OemToAnsi("�tem selecionado e codigo"),;
																	 						OemToAnsi("�tem selecionado e descricao"),;
																	 						OemToAnsi("�tem nao selecionado e codigo"),;
																	 						OemToAnsi("�tem nao selecionado e descricao");
...
/*
�����������������������������������������������������������������������Ŀ
�Funcao    �AdmOpcPsq	    �Autor�Marinaldo de Jesus � Data �11/09/2003�
�����������������������������������������������������������������������Ĵ
�Descricao �Pesquisar as Opcoes em AdmOpcoes                      		�
�����������������������������������������������������������������������Ĵ
�Sintaxe   �<Vide Parametros Formais>									�
�����������������������������������������������������������������������Ĵ
�Uso       �AdmOpcoes()                                                 �
...
DEFINE MSDIALOG oDlg TITLE OemToAnsi(cTitulo) From aAdvSize[7],0 TO aAdvSize[6]+20,aAdvSize[5] OF GetWndDefault() PIXEL
	...
	@ ( aObjSize[1,1] + 010 ) , ( aObjSize[1,2]+100 )	RADIO oRadio VAR nOpcRad	ITEMS 	OemToAnsi("codigo"),;
																	 						OemToAnsi("descricao") ;
																						SIZE 115,010 OF oDlg PIXEL
	oRadio:cToolTip := OemToAnsi( "Ap�s selecionar pressione a tecla <TAB> para habilitar a digitacao" )
	oRadio:oFont	:= oFont
...
User Function BxTitFid(_cChvBx)
	...
	If  SE1->(DbSeek(_cChvBx))
		...
		if SE1->E1_SALDO = 0 .AND. ! Empty(SE1->E1_BAIXA)
			ShowHelpDlg("NOTIT", {"Titulo "+SE1->E1_PREFIXO+"-"+SE1->E1_NUM+"-"+SE1->E1_PARCELA+ " j� encontra-se baixado",""},3,;
			{"Verificar situacao do titulo junto a �rea Financeira",""},3)
			lRet := .F.
		Else
			lMsErroAuto := .F.
...
Solicitante:            Cliente
Uso......:              
Obs......:              Permitir o usr selecionar o BANCO para defini��o de Par�metros
======================================================================================
*/
...
Local aRet 		:= {}
Local nRetBco	:= 1
Local cDesc1	:= GETMV("MGF_FIN43A")				// "341-Ita�"
Local cDesc2	:= GETMV("MGF_FIN44A") 				// "237-Bradesco"

aAdd( aPergs ,{3,"Defina o Parametro Padr�o:",1, {Alltrim(cDesc1), Alltrim(cDesc2)}, 80,'.T.',.T.})   
