...
User Function MGFCRM12(xRet)
    ...
    +---------------------------------------------------------------*/
    MsgAlert( OEMToANSI( "A tecla [F4] - busca unit�ria do RAMI - sera descontinuada em breve. Passe a usar a nova funcionalidade [F2]: selecao m�ltipla de RAMI." ) )
	...
	if cFormul == "S"
		...
		empty(dDEmissao)
			...
			msgAlert("Para associar a RAMI � necessario preencher o cabecalho.")
			return
		endif
	elseif cFormul == "N"
		if empty(cA100For) .or.;
	...
	if paramBox(aParambox, "Relatorio de An�lise de Mercado Interno"	, @aRet, { || !empty(MV_PAR01) }, , .T. /*lCentered*/, 0, 0, , , .F. /*lCanSave*/, .F. /*lUserSave*/)
		if INCLUI
...
Data.....:              26/12/2017
Descricao / Objetivo:   Valida�o para X3_WHEN dos campos
Doc. Origem:            GAP CRM
Solicitante:            Cliente
Uso......:              
Obs......:
...
Autor....:              wanderley.silva@totvs.com.br
Data.....:              26/12/2017
Descricao / Objetivo:   Valida�o para nao permitir altera�ao do codigo
Doc. Origem:            GAP CRM
Solicitante:            Cliente
Uso......:              
Obs......:
...
Local  l_Ret    :=  .T.
Local  aArea    :=  GetArea()
Local  c_Var    :=  ReadVar() // Obt�m o nome da variavel
Local  c_Cont   :=  &( ReadVar() ) // Obt�m o conteudo da variavel
Static c_Cod

If AllTrim( M->cTipo ) == "D" .and. !IsInCallStack( "LOJA720" )
   If n_Type == 1
      c_Cod := &( ReadVar() ) // Obt�m o conteudo da variavel aCols[n, n_PosCod]
   ElseIf AllTrim( c_Var ) == "M->D1_COD"
          If &c_Var <> c_Cod //  >>>-----> Tentativa de trocar ou inserir codigo de produto no D1_COD.
             If .not. Empty( c_Cod )
                ...
                &c_Var  :=  c_Cod
             Else
                MsgAlert( OEMToANSI( "Inser��o novo produto nao permitido para Devolucao (com RAMI)." ) )
   ...
   ElseIf AllTrim( c_Var ) == "M->D1_QUANT" .or. AllTrim( c_Var ) == "M->D1_VUNIT"
          If &c_Var > c_Cod //  >>>-----> Tentativa de inserir valor maior no campo D1_QUANT ou D1_VUNIT.
             MsgAlert( OEMToANSI( "O Valor Unit�rio e/ou Quantidade nao deve ser maior na Devolucao (com RAMI)." ) )
             &c_Var  :=  c_Cod
             l_Ret   :=  .F.
          EndIf
   EndIf
...
//-----------------------------------------------------------------
// Verifica se RAMI digitada j� foi utilizada anteriormente
//-----------------------------------------------------------------
static function chkRami()
	...
	if !QRYSD1->(EOF())
		msgAlert("RAMI " + allTrim(MV_PAR01) + " j� utilizada em outra devolucao.")
		lRet := .F.
	endif
