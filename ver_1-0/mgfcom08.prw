...
User Function MC8ValPo()
	...
	If UPPER(AllTrim(FUNNAME())) == 'MATA110'
		...
		If Empty(cGrp)
			lRet := .F.
			Alert('S� � Permitido inclusao de produtos na solicitacao que possuam Grupo')
		EndIf
	...
	If Empty(('ZCUSTGRADE')->CCUSTO) .OR. Empty(('ZCUSTGRADE')->GRUPO)
		...
		If !(Empty(cCC)) .AND. !(Empty(cGrp))
			If !(Empty(cCC)) .and. !(Empty(cGrp))
				...
				If !lRet
					If IsBlind()
						Help(" ",1,'SEMGRADE',,'Nao  existe grade t�cnica cadastrada para este CENTRO DE CUSTO + GRUPO DE PRODUTOS.'+CRLF+;
						'Esta SC nao sera gravada. Entre em contato com o respons�vel pelo cadastramento da grade e informe esta mensagem.',1,0)
					Else
						Alert('Nao  existe grade t�cnica cadastrada para este CENTRO DE CUSTO (' + Alltrim(cCC) + ') + GRUPO DE PRODUTOS (' + Alltrim(cGrp) + '). Esta SC nao sera gravada. Entre em contato com o respons�vel pelo cadastramento da grade e informe esta mensagem.')
					EndIf
				EndIf
			EndIf
		EndIf
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Apos a gravacao da alcada de aprovacao � gravado a alcada correta
=====================================================================================
*/
...
Autor...............: Joni Lima
Data................: 25/01/2016
Descricao / Objetivo: Realiza a prepara��o dos dados para envio ao Fluig
Doc. Origem.........: GRADE ERP
Solicitante.........: Cliente
Uso.................: 
...
User Function xMC8GSCR()
	...
	If SE2->E2_ZNEXGRD == 'S'
		...
		EndIf
	Else
		Alert('S� � possivel utilizar essa rotina para titulos que em sua geracao/alteracao nao possu�am grade de aprovacao.')
	EndIf
...
Obs.................: Valida se o campo tipo � igual ao MPA
=====================================================================================
*/
...
// Faz a mesma funcao que a u_xM8verCC para as medi��es de contrato
//---------------------------------
...
//---------------------------------
// Retorna CC da Medi��o posicionada
//---------------------------------
	...
	If select ('ZCUSTGRADE') > 0
		...
		If !lRet
			If IsBlind()
				...
				'Esta SC nao sera gravada. Entre em contato com o respons�vel pelo cadastramento da grade e informe esta mensagem.',1,0)
			Else
				Alert('Nao  existe grade t�cnica cadastrada para este CENTRO DE CUSTO (' + Alltrim(cCC) + ') + GRUPO DE PRODUTOS(' + Alltrim(cGR) + ').Esta SC nao sera gravada. Entre em contato com o respons�vel pelo cadastramento da grade e informe esta mensagem.')
			EndIf
		EndIf
	else
		lRet := .T.
...
User Function xPAEIC()
	...
	Else
		...
		If (Alltrim(SE2->E2_TIPO) == "PA") .And. (Alltrim(SE2->E2_PREFIXO) == "EIC") .And. (Alltrim(SE2->E2_ORIGEM) == "SIGAEIC") // .And. (Alltrim(_cEstado)=="EX")
			...
			*------------------------ Bot�es -------------------------------------------------*
			@ 045,200 BUTTON "Processar" SIZE 35,15 ACTION (AltVenc(_dVenCto, _dVenRea), Close(oDlg1))	// Bot�o ativando Funcao que ira gravar os Vencimentos Informados
			@ 065,200 BUTTON "Sair"      SIZE 35,15 ACTION Close(oDlg1)									// Bot�o Fechando a Janela
			*---------------------------------------------------------------------------------*
		...
		Else
			*----------------- Nao  Condiz com a Regra de Permissao PA-EIC --------------------*
			If (Alltrim(SE2->E2_TIPO) 		<> "PA")
				_cMsg:= "pois o Tipo �: " 		+ Alltrim(SE2->E2_TIPO)
			EndIf
			If (Alltrim(SE2->E2_PREFIXO) 	<> "EIC")
				_cMsg:= "pois o Prefixo �: " 	+ Alltrim(E2_PREFIXO)
			EndIf
			If Alltrim(SE2->E2_ORIGEM) 		<> "SIGAEIC"
				_cMsg:= "pois a Origem �: " 	+ Alltrim(E2_ORIGEM)
			EndIf
...
//Descricao : Funcao Para Grava��o dos campos de Vencimentos em SE2 e em SWB, chamada pela funcao xPAEIC()
//			  Bot�o Processar do Dialog em xPAEIC().
@author Andy Pudja
@since 08/11/2018
@version 1.0
@return ${return}, ${return_description}
...
Local _lGraSWB	:= .F.
		...
		*--------- Aviso do Processo ----------*
		...
		Else
			If _lGraSE2
				Alert('Vencimentos dos Titulos Foram Atualizados, Exceto Registro de �tem de C�mbio (SWB) !!! ')
			Else
				If !_lGraSWB
					...
					// pois o titulo sempre existir� e a rotina esta posicionada!
					// No entanto vamos deixar a estutura da consist�ncia como esta para a quest�o de entendimento da possibilidade dos cen�rios.
				EndIf
			EndIf
		EndIf
		*--------------------------------------*
