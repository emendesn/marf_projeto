...
//����������������������������������������������������������������������������������������������������������������������������������������������������
//������������������������������������������������������������������������������������������������������������������������������������������������ͻ��
//���Programa  � MGF29R22	�Autor  �Geronimo Benedito Alves                                                                    �Data �  10/01/18  ���
//������������������������������������������������������������������������������������������������������������������������������������������������͹��
//���Desc.		� Rotina que mostra na tela os dados da planilha: EEC-Easy Export Control - 29 -EXPORTA��O - Relatorio Estufagem                   ���
...
User Function MGF29R22()
	...
	Aadd(_aDefinePl, "Exportacao - Relatorio Estufagem"	)	//01-	_cTitulo	- Titulo da planilha a ser gerada. Aparecer� na regua de processamento.
	Aadd(_aDefinePl, "Relatorio Estufagem"				)	//02-	_cArqName  - Nome da planilha Excel a ser criada
	Aadd(_aDefinePl, {"Relatorio Estufagem"}			)	//03-	_cNomAbAna - Titulo(s) da(s) aba(s) na planilha excel
	Aadd(_aDefinePl, {"Relatorio Estufagem"}			)	//04-	_cNomTTAna - Titulo(s) da(s) tabela(s) na planilha excel
	...
	Aadd(_aCampoQry, {"ZZR_PERDE"	,"DATA_PRODUCAO_DE			as DTPROD_DE", 	"Data Produ��o De"		,"D",008	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"ZZR_PERATE"	,"DATA_PRODUCAO_ATE			as DTPROD_ATE"	,"Data Produ��o Ate"	,"D",008	,0	,""						,""		,""	})
	...
	Aadd(_aCampoQry, {"A1_COD"		,"CODIGO_BUYER				as COD_BUYER"	,"C�d. Buyer"			,"C",006	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"A1_NOME"		,"NOME_BUYER"								,"Nome Buyer"			,"C",040	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"A1_COD"		,"CODIGO_CONSIGNEE			as COD_CONSIG"	,"C�d. Consignee"		,"C",006	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"A1_NOME"		,"NOME_CONSIGNEE			as NOM_CONSIG"	,"Nome Consignee"		,"C",040	,0	,""						,""		,""	})
	Aadd(_aCampoQry, {"A1_COD"		,"CODIGO_IMPORTADOR			as COD_IMPORT"	,"C�d. Importador NF"	,"C",006	,0	,""						,""		,""	})
	...
	Aadd(_aCampoQry, {"YA_DESCR"	,"PAIS_DESTINO				as PAIS_DESTI"	,"Pa�s Destino"			,"C",025	,0	,""						,""		,""	})
	...
	aAdd(_aParambox,{1,"Numero de Exportacao"		,Space(tamSx3("EEC_ZEXP")[1])	,"@!"	,""														,"EECEXP"	,"",050,.F.})
	aAdd(_aParambox,{1,"C�d Importador Inicio"		,Space(tamSx3("A1_COD")[1])		,""		,""														,"CLI"		,"",050,.F.})
	aAdd(_aParambox,{1,"C�d Importador Final "		,Space(tamSx3("A1_COD")[1])		,""		,"U_VLFIMMAI(MV_PAR02,MV_PAR03,'C�d. Importador')"		,"CLI"		,"",050,.F.})
	...
	_aSelFil:= U_SeleEmp()					// Rotina para obter a selec�o das EMPRESAS a processar.
	If Empty(_aSelFil) ; Return ; Endif
	_cCODEMPRE  := U_Array_In(_aSelFil)

	_cQuery += "  FROM " + U_IF_BIMFR( "IF_BIMFR", "V_EX_RELESTUFAGEMKARIN"  )  + CRLF
	_cQuery += U_WhereAnd( !empty(_cCODEMPRE ),  " FILIAL_FILTRO IN "           + _cCODEMPRE                             ) //OBRIGATORIO (SELE��O DO COMBO)  CAMPO EMPRESAS(02 posicoes)
	_cQuery += U_WhereAnd( !empty(_aRet[1] ),    " NUM_EXP LIKE '%"             + _aRet[1] + "%' "                       ) 
	_cQuery += U_WhereAnd( !empty(_aRet[3] ),    " CODIGO_IMPORTADOR BETWEEN '" + _aRet[2] + "' AND '" + _aRet[3] + "' " ) //NAO OBRIGATORIO, USUARIO COLOCA O CODIGO DE/ATE (RANGE)
	_cQuery += U_WhereAnd( !empty(_aRet[4] ),    " ADMINISTRADOR LIKE '%"       + _aRet[4] + "%' "                       ) //NAO OBRIGATORIO (USUARIO DIGITA )
	_cQuery += U_WhereAnd( !empty(_aRet[6] ),    " ESTUFAGEM_PCP BETWEEN '"     + _aRet[5] + "' AND '" + _aRet[6] + "' " ) //NAO OBRIGATORIO
