...
*/
//-------------------------------------------------------------------
// Para ser chamado em JOB com várias filiais
//-------------------------------------------------------------------
user function MGFWSC27()

Local _afiliais := {}

U_MFCONOUT("Iniciando ambiente de exportação de estoques locais para o E-Commerce")

RPCSetType( 3 )
PREPARE ENVIRONMENT EMPRESA '01' FILIAL '010041'

_afiliais := strtokarr2(getmv("MGFWSC27F",,"010016,010041,010044,010050,010066"),",")

For _nnt := 1 to len(_afiliais)

	cfilant := _afiliais[_nnt]
	_cfilial := cfilant
	U_MFCONOUT('Iniciando processamento de estoque local para filial ' + _CFILIAL + '...')
	MGFWSC27E(_CFILIAL)
	U_MFCONOUT('Completou processamento de estoque local para filial ' + _CFILIAL + '...')
...
static function MGFWSC27E( )
	...
	local cTimeFin		:= ""
	local cTimeProc		:= ""
	local cURLPostori	:= allTrim( superGetMv( "MGFECOM27A" , , "http://spdwvapl219:8206/protheusestoque/api/inventario/" ) )
	...
	while !QRYSB2->(EOF())
			...
			_nni++
			U_MFCONOUT('Exportando estoque do produto ' + allTrim( QRYSB2->IDPRODUTO ) + " - " + strzero(_nni,6) + " de " + strzero(_ntot,6) + "...")
			oJson := nil
			...
			cURLPost	:= ""
			cURLPost	:= cURLPostori + allTrim( QRYSB2->IDPRODUTO )
			...
			if !empty( cJson )
				...
				conout(" [E-COM] [MGFWSC27] * * * * * Status da integracao * * * * *")
				conout(" [E-COM] [MGFWSC27] Inicio.......................: " + cTimeIni + " - " + dToC(dDataBase))
				conout(" [E-COM] [MGFWSC27] Fim..........................: " + cTimeFin + " - " + dToC(dDataBase))
				conout(" [E-COM] [MGFWSC27] Tempo de Processamento.......: " + cTimeProc)
				conout(" [E-COM] [MGFWSC27] URL..........................: " + cURLPost)
				conout(" [E-COM] [MGFWSC27] HTTP Method..................: " + "PUT")
				conout(" [E-COM] [MGFWSC27] Status Http (200 a 299 ok)...: " + allTrim( str( nStatuHttp ) ) )
				conout(" [E-COM] [MGFWSC27] Envio........................: " + cJson)
				conout(" [E-COM] [MGFWSC27] * * * * * * * * * * * * * * * * * * * * ")
...
static function setStock()
	...
	oJson['bfilial']		:= allTrim( cFilAnt )
	oJson['produto']		:= allTrim( QRYSB2->idproduto )
	oJson['bqtdeestoque']	:= 0

	aRetSaldo := {0,0}
	aRetSaldo := getSalProt(oJson['produto'], "", cFilAnt, .T.)

	if aRetSaldo[1] > 0
		oJson['bqtdeestoque'] := ( ( nPorcEcom / 100 ) * aRetSaldo[1] / QRYSB2->B1_ZPESMED )
	endif

	if oJson['bqtdeestoque'] < 1
		oJson['bqtdeestoque'] := 0
	else
		oJson['bqtdeestoque'] := round( oJson['bqtdeestoque'], 0 )
	endif
return
...
static function getStock()
	...
	cQRYSB2 := " SELECT DISTINCT DA1_FILIAL, DA1_CODPRO IDPRODUTO, 'EC' idtipopedi, 'N' ZJ_FEFO, COALESCE( B1_ZPESMED, 0 ) B1_ZPESMED"						+ CRLF
...
static function getSalProt( cB1Cod, cC5Num, cStockFil, lJobStock, dDtMin, dDtMax, _BlqEst )
	...
	if lJobStock
		if QRYSB2->ZJ_FEFO <> 'S'
			MFWSC27D( @aRet, cStockFil, QRYSB2->idproduto, .F., dDataMin, dDataMax )
		else
			...
			aRet2 := {0}
			MFWSC27D( @aRet2, cStockFil, QRYSB2->idproduto, .T., dDataMin, dDataMax )
		...
		if SZJ->ZJ_FEFO <> 'S'
			if !empty( dDataMin ) .and. !empty( dDataMax )
				MFWSC27D( @aRet, cStockFil, cB1Cod, .T., dDataMin, dDataMax )
				...
				aRet2 := {0}
				MFWSC27D( @aRet2, cStockFil, cB1Cod, .F., dDataMin, dDataMax )
				...
				MFWSC27D( @aRet, cStockFil, cB1Cod, .F., dDataMin, dDataMax )
			endif
		else
			// Se NAO houver saldo no VE outros tipos recebem o saldo do VE (zerado ou negativo)
			// Se HOUVER saldo no VE consulta novamente o saldo nos outros tipos
			...
			aRet2 := {0}
			MFWSC27D( @aRet2, cStockFil, cB1Cod, .T., dDataMin, dDataMax )
	...
	Conout("[MGFWSC27] - Resuldado da funcao getSalProt: Saldo: "+ Alltrim(Transform(nSalProt,"@E 999,999,999.9999")) + " Peso Medio: "+ Alltrim(Transform(nPesoMedio,"@E 999,999,999.9999")) )
return aRetStock
...
static function getSaldoPv( cB1Cod, cStockFil, cC5Num, dDataMin, dDataMax, _BlqEst )
	...
	Conout("[MGFWSC27] - Roda Query funcao getSaldoPv: "+ cQueryProt )
	...
	If Select(cQryPv) > 0
		(cQryPv)->(DBCloseArea())
	EndIf
	Conout("[MGFWSC27] - Resuldado da Query funcao getSaldoPv saldo pvs: "+ Transform(nSaldoPV,"@E 999,999,999.9999") )
	...
	if _BlqEst
		...
		If Select(cQryPv) > 0
			(cQryPv)->(DBCloseArea())
		EndIf
		Conout("[MGFWSC27] - Resuldado da Query funcao getSaldoPv - saldos bloqueados: "+ Transform(nSaldoPV,"@E 999,999,999.9999") )
	ENDIF
...
=====================================================================================
*/
Static Function MFWSC27D(xRet,xFilProd,xProd,xFEFO,xDTInicial,xDTFinal)
              //MFWSC27D( @aRet, cStockFil, cB1Cod, .F., dDataMin, dDataMax )
