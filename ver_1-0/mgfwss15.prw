...
Autor....:              Rafael Garcia
Data.....:              17/10/2019
Descricao / Objetivo:   WS Integracao PROTHEUS x MultiEmbarcador - Recebimento NFS 
						Gera Frete Combinado, simula integracao do cte, libera Romaneio, 
						execAuto liberacao de Romaneio e processamento.
						Gera NFS sobre ocorrências.
Doc. Origem:            Especificacao de Processos_Emissao_CT-e - EF - 01_v2
Solicitante:            Cliente
Uso......:              Marfrig
Obs......:              WS  Recebimento NFS
=====================================================================================
*/
...
WSSTRUCT MGFWSS15REC
	...
	WSDATA numeroDocumento		as string
	WSDATA ProtocoloNFSe		as string Optional
	WSDATA OcorrenciaMulti		as string Optional
	...
	WSDATA AliquotaISS			as float
	WSDATA ValorISS				as float
	WSDATA codigoVerificacao	as string
	WSDATA ValorBaseCalculoISS	as float
	WSDATA ValorRetencaoISS		as float
	WSDATA PercentualImpRetido	as float
	WSDATA quantidadeDocumentos	as float
	WSDATA ValorCarga			as float

	WSDATA NotaFiscal			as array of NotaFiscal

ENDWSSTRUCT

WSSTRUCT MGFWSS15RET
...
***************************************************************************/
WSSERVICE MGFWSS15 DESCRIPTION "Integracao Protheus x Multiembarcador - NFS" NameSpace "http://totvs.com.br/MGFWSS15.apw"

	// Passagem dos parametros de entrada
	WSDATA MGFWSS15RECEBER AS MGFWSS15REC
	// Retorno (array)
	WSDATA MGFWSS15RETORNO   AS MGFWSS15Ret

	WSMETHOD RetNFS DESCRIPTION "Integracao Protheus x Multiembarcador - Retorno NFS"
...
************************************************************************************/
WSMETHOD RetNFS WSRECEIVE MGFWSS15RECEBER WSSEND MGFWSS15RETORNO WSSERVICE MGFWSS15

	Local aRetFuncao := {}
	Local lReturn	:= .T.

	aRetFuncao	:= u_MGFWSS15(::MGFWSS15RECEBER)	// Passagem de parametros para rotina

	// Cria e alimenta uma nova instancia de retorno do cliente
	::MGFWSS15RETORNO 				:=  WSClassNew( "MGFWSS15Ret" )
	::MGFWSS15RETORNO:status		:= aRetFuncao[1][1]
	::MGFWSS15RETORNO:Msg			:= aRetFuncao[1][2]

Return lReturn


user function MGFWSS15( oNFS )
	Local aRet     	:= {}
	local cFil 	   	:= ""		
	Local aArea	   	:= GetArea()
	...
	If !ExisteSx6("MGF_WSS15")
		CriarSX6("MGF_WSS15", "L", "Determina se Aliq.Iss será zerada se valor Iss for zero", ".T." )
	EndIf

	_lWSS15  := SuperGetMv('MGF_WSS15'  , NIL, '')		
	cFil	 := AllTrim(QFIL(oNFS:cnpjFilial))
	_cNroDoc := StrZero(Val(oNFS:numeroDocumento),9)

	If !Empty(oNFS:OcorrenciaMulti)
		_cOcorMult := AllTrim(oNFS:OcorrenciaMulti)
	EndIf
	...
	Else
		...
		_cSerNFS	:= SubStr(AllTrim(Str(Year(oNFS:dataEmissao))),2,3) 
		...
		BeginSql Alias _cGXG
			...
			WHERE 
				GXG.%notdel% 									AND
				GXG.GXG_FILIAL	=  %Exp:cFil%					AND
				GXG.GXG_CDESP	=  %Exp:oNFS:Especie%			AND
				GXG.GXG_EMISDF	=  %Exp:oNFS:Transportador%		AND
				GXG.GXG_SERDF	=  %Exp:_cSerNFS%				AND
				GXG.GXG_NRDF	=  %Exp:oNFS:numeroDocumento%	AND
				GXG.GXG_DTEMIS	=  %Exp:oNFS:dataEmissao%
			ORDER BY GXG_FILIAL, GXG_CDESP, GXG_EMISDF, GXG_SERDF, GXG_NRDF, GXG_DTEMIS  

		EndSql
		//MemoWrite("C:\QRY\WSS15GXG.SQL",GetLastQuery()[2])

		If (_cGXG)->( Eof())
			If oNFS:valorRetencaoISS > 0
				_cTribImp := "1"
			Else
				_cTribImp := "2"
			EndIf

			If	_lWSS15 .and. _cTribImp = "2"
				_nPcImp := 0
				_nVlImp := 0
				_nBsImp := 0
			Else
				_nPcImp	:= oNFS:AliquotaISS
				_nVlImp	:= oNFS:ValorISS
				_nBsImp := oNFS:ValorBaseCAlculoISS
			Endif

			If oNFS:TipoDocto <> "7"	//Se não for NFS de Ocorrências, gerar Frete Combinado
				aRet	:= xfreteC(oNfs) //Execauto para inclusao do frete combinado
			Else
				aadd(aRet,{"1","Sucesso-NFS Ocorrencia"})
			EndIf
			...
			If aRet[1][1]=="1"
				...
				RecLock("GXG",.T.)
					...
					GXG->GXG_ACAO  	:= oNfs:Acao
					GXG->GXG_EDINRL	:= VAL(oNfs:NrLinha) 
					GXG->GXG_EDIARQ	:= "INTEGRACAO-NFS"
					GXG->GXG_DTIMP	:= DATE()
					GXG->GXG_ALTER  := oNFS:Alterado
					GXG->GXG_EDISIT := oNFS:SitEdi
					GXG->GXG_FILDOC := xFilial("GXG")
					GXG->GXG_CDESP	:= oNFS:Especie
					GXG->GXG_EMISDF := oNFS:Transportador
					GXG->GXG_SERDF 	:= _cSerNFS
					GXG->GXG_ORIGEM := oNFS:Origem
					GXG->GXG_NRDF   := oNFS:numeroDocumento
					GXG->GXG_DTEMIS := oNFS:dataEmissao
					GXG->GXG_CDREM  := oNFS:cnpjRemetente
					GXG->GXG_TPDF   := oNFS:TipoDocto
					GXG->GXG_DTENT  := DATE()
					GXG->GXG_CFOP   := oNFS:CFOP
					GXG->GXG_VLDF   := oNFS:valorDocumento
					GXG->GXG_TRBIMP := _cTribImp
					GXG->GXG_PCIMP	:= _nPcImp
					GXG->GXG_VLIMP	:= _nVlImp
					GXG->GXG_BASIMP := _nBsImp
					GXG->GXG_IMPRET := oNFS:valorRetencaoISS
					GXG->GXG_QTDCS  := oNFS:QuantidadeDocumentos
					GXG->GXG_VLCARG := oNFS:valorCarga
					GXG->GXG_USUIMP := "INTEGRACAO"
					GXG->GXG_XCODV  := oNFS:codigoVerificacao

					If !Empty(oNFS:ProtocoloNFSe)
						GXG->GXG_XPROTO := oNFS:ProtocoloNFSe
					EndIf
					
				MsUnlock("GXG")

				For _xI:=1 To LEN(oNFS:NotaFiscal)
					//-----| Gerar Carga do doc Frete |-----
					DBSELECTAREA("GW1")
					DBSETORDER(12)
					If DBSEEK(alltrim(oNFS:NotaFiscal[_xI]:ChaveNFe))
						...
						RecLock("GXH", .T.)
							...
							GXH->GXH_SEQ  	:= padl(ALLTRIM(STR(_xI)),TamSX3("GXH_SEQ" )[1],"0")
							...
							GXH->GXH_DANFE 	:= alltrim(oNFS:NotaFiscal[_xI]:ChaveNFe)
						MsUnlock("GXH")

						//-----| Guardar Documentos do calculo de frete |----- 
						aAdd(_aDocFrt, {cTipoDC,GW1->GW1_EMISDC,GW1->GW1_SERDC,GW1->GW1_NRDC}) 

					EndIf
				Next

				If oNFS:TipoDocto <> "7"	//Se não for NFS de Ocorrências, gerar Carga do doc Frete e liberar romaneio.	
					//-----| Liberar Romaneio |-----
					dbSelectArea("GWN")
					dbSetOrder( 1 )
					If dbSeek( GW1->GW1_FILIAL+GW1->GW1_NRROM )
						If GWN->GWN_SIT <> "3"
							GFEA050LIB(.T.,"",DDATABASE,SubStr(Time(), 1, 5)) 
						EndIf
					EndIf

					//-----| Altera dados do ISS |-----
					dbSelectArea("GWF")
					dbSetOrder(4)	//Romaneio         
					If dbSeek(GW1->GW1_FILIAL+GW1->GW1_NRROM)	
						While !Eof() .AND. GWF->GWF_NRROM = GW1->GW1_NRROM
							If Empty(GWF->GWF_NRDF)	
								GWF->(RecLock("GWF",.F.))
									GWF->GWF_TPTRIB	:= _cTribImp
									GWF->GWF_BASISS := _nBsImp
									GWF->GWF_PCISS	:= _nPcImp
									GWF->GWF_VLISS	:= _nVlImp
								GWF->(MSUNLOCK())
							EndIf
							GWF->( dbSkip() )
						EndDo 	
					EndIf
				Else
					//-----| Relacionar Doc.Frete x Calculo de Frete |-----
					For Y:=1 To LEN(_aDocFrt)
						dbSelectArea("GWH")
						dbSetOrder(2)	//Filial + Tipo Docto + Emissor DC + Serie + Numero DC + Calculo
						If dbSeek( xFilial("GWH")+_aDocFrt[Y,1]+_aDocFrt[Y,2]+_aDocFrt[Y,3]+_aDocFrt[Y,4] )
							While !Eof() .AND. GWH->GWH_FILIAL  = xFilial("GWH") .AND. GWH->GWH_CDTPDC = _aDocFrt[Y,1] .AND. ;
									GWH->GWH_EMISDC = _aDocFrt[Y,2] .AND. GWH->GWH_SERDC = _aDocFrt[Y,3] .AND. GWH->GWH_NRDC = _aDocFrt[Y,4] 
								//-----| Altera dados do ISS |-----
								dbSelectArea("GWF")
								dbSetOrder(1)	//Calculo        
				...
				xGFEA115PR("3",.t.,cNRIMP) 

			EndIf
		Else
			aadd(aRet,{"2","NFS JA IMPORTADA ANTERIORMENTE"})
		EndIf
	EndIf
...
static FUNCTION xFreteC(oNfs)
	...
	cfilant:=QFIL(oNFS:CNPJRemetente)

	/*/
	{"GWF_CDESP"  	,oNFS:Especie					,NIL},;
	{"GWF_EMISDF"  	,oNFS:Transportador				,NIL},;
	{"GWF_SERDF"  	,_cSerNFS		  				,NIL},;
	{"GWF_NRDF"  	,_cNroDoc  						,NIL},;
	{"GWF_DTEMDF"  	,oNFS:dataEmissao				,NIL},;
	/*/

	aGWF:=  {;
			{"GWF_FILIAL" 	,XFILIAL("GWF")					,NIL},;
			{"GWF_TPTRIB"  	,_cTribImp 						,NIL},;
			{"GWF_BASISS"  	,_nBsImp 						,NIL},;
			{"GWF_PCISS"  	,_nPcImp		  				,NIL},;
			{"GWF_VLISS"  	,_nVlImp			  			,NIL},;
			{"GWF_IMPOST"  	,2								,NIL},;
	...
	aGWI:= {{"GWI_FILIAL" ,XFILIAL("GWI")		,NIL},;
			...
			{"GWI_VLFRET" ,onfs:ValorDocumento	   }}

	FOR I:=1 TO LEN(oNFS:NotaFiscal)

		DBSELECTAREA("GW1")
		DBSETORDER(12)
		IF DBSEEK(oNFS:NotaFiscal[I]:ChaveNFe)
	...
	If !lRet
		...
		cMsg:= "Id do formulario de origem:" + ' [' + AllToChar( aErro[1]  ) + ']' 
		cMsg+= "Id do campo de origem:     " + ' [' + AllToChar( aErro[2]  ) + ']' 
		cMsg+= "Id do formulario de erro:  " + ' [' + AllToChar( aErro[3]  ) + ']' 
		cMsg+= "Id do campo de erro:       " + ' [' + AllToChar( aErro[4]  ) + ']' 
		cMsg+= "Id do erro:                " + ' [' + AllToChar( aErro[5]  ) + ']' 
		cMsg+= "Mensagem do erro:          " + ' [' + AllToChar( aErro[6]  ) + ']' 
		cMsg+= "Mensagem da solucao:       " + ' [' + AllToChar( aErro[7]  ) + ']' 
		cMsg+= "Valor atribuido:           " + ' [' + AllToChar( aErro[8]  ) + ']' 
		cMsg+= "Valor anterior:            " + ' [' + AllToChar( aErro[9]  ) + ']' 
		aadd(aRet,{"2",cMsg})
	EndIf
	// Desativamos o Model
	oModel:DeActivate()
