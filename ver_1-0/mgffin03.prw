...
Static Function fGeraQry(	cFields		,; // String contendo os campos que serao utilizados para a query e o aHeader da MarkBrowse
	...
	// 13/03/2018 - Atilio - Incluir tratamento para data = feriado. Incluir pr�ximo dia util (resultado de DataValida())
	...
	If ( lRet )
		...
		Do Case
			Case ( lExistCod )
				MsgAlert('Este Codigo de barra j� existe no Titulo : ' +  cCodTit)
				cCodBarra := Space(50)
				aDados	  := Array(1,15)
			Case !( lDisp )
				MsgAlert('Nao existem Titulos dispon�veis para este Codigo de barras')
				cCodBarra := Space(50)
				aDados	  := Array(1,15)
		EndCase
...
Static FUNCTION fConVld(cCodBarra)
	...
	IF VALTYPE(cCodbarra) == NIL .OR. EMPTY(cCodbarra)
		...
		cStr := ""
	ELSE
		// Se o Tamanho do String for menor que 44, completa com zeros ate 47 digitos. Isso �
		// necessario para Bloquetos que nao t�m o vencimento e/ou o valor informados na LD.
...
//|				Geral (DVG) que � consistido pelo Modulo 11. Essa LD t�m 47  |
//|            Digitos.                                                      |
//|                                                                          |
...
// Se o Tamanho do String for 45 ou 46 esta errado! Retornar� .F.
...
// Verifica se todos os digitos sao num�rios.
FOR i := LEN(cStr) TO 1 STEP -1
	lRet := IF(SUBSTR(cStr,i,1) $ "0123456789",lRet,.F.)
NEXT

IF LEN(cStr) == 47 .AND. lRet
	// Consiste os tres DV�s de Bloquetos pelo Modulo 10.
	nConta  := 1
	WHILE nConta <= 3
		nMult  := 2
		nVal   := 0
		...
		nConta  := nConta + 1
	ENDDO
	// Se os DV�s foram consistidos com sucesso (lRet=.T.), converte o numero para CB para consistir o DVG.
...
IF LEN(cStr) == 48 .AND. lRet
	// Consiste os quatro DV�s de Titulos de Concessionarias de Servico Publico e IPTU pelo Modulo 10.
	nConta  := 1
	WHILE nConta <= 4
		nMult  := 2
		nVal   := 0
...
IF LEN(cStr) == 44 .AND. lRet
	IF cTipo $ "BI"
		...
		lRet    := IF(lRet,(nDVCalc==nDV),.F.)
		// Se o Tipo � (I)ndefinido E o DVG nao foi consistido com sucesso (lRet=.F.), tentara
...
// (GPS � validado por este modulo e possui 48 posicoes)
...
If !( lRet )
	MsgAlert('Codigo de barras inv�lido')
EndIf
...
Data.....:            15/08/2018
Descricao / Objetivo: Verifica se o Codigo de barras j� existe
Doc. Origem:          Contrato - GAP MGFFIN03
Solicitante:          Cliente
Uso......:            
...
// Verifica se j� existe o Codigo de barras
dbSelectArea("SE2")
dbSetOrder(22) // E2_CODBAR
If !Empty(cCodBar) .AND. dbSeek(cCodBar)
   ApMsgAlert(OemToAnsi('Codigo de barra j� existe no Titulo : '+SE2->E2_NUM),OemToAnsi('ATENCAO'))
   lRet := .T.
EndIf   			

// Verifica se j� existe a linha digitavel tamb�m
dbSelectArea("SE2")   
dbSetOrder(23) // E2_LINDIG
If !Empty(cCodBarra) .AND. dbSeek(cCodBarra)
   ApMsgAlert(OemToAnsi('Linha digitavel j� existe no Titulo : '+SE2->E2_NUM),OemToAnsi('ATENCAO'))
