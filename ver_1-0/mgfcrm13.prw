...
user function MGFCRM13()
	...
	if INCLUI .or. ALTERA
		...
		for nI := 1 to len(aCols)
			if !aCols[nI, len(aHeader)+1]
				...
				if !lOk
					lRet := .F.
					cStrLog += "Item " + allTrim(aCols[nI, nPosItem]) + " da Pr� nota nao corresponde com o RAMI." + CRLF
				endif
			endif
		next
		...
		while !QRYZAW->(EOF())
			...
			if !lOk
				lRet := .F.
				cStrLog += "Item " + allTrim(QRYZAW->ZAW_ITEMNF) + " do RAMI nao foi lan�ado na Pr� nota." + CRLF
			endif
	...
	if !empty(cStrLog)
		aviso("Pr� Nota de Devolucao inv�lida", "A pr� nota de devolucao digitada � inv�lida. " + CRLF + "Verifique problemas abaixo: " + CRLF + cStrLog, {"Ok"}, 3)
	else
		M->F1_ZSENHA := cCdRami
	endif
