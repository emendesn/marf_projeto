...
static function CRM27POS( oModel )
	...
	if nOper == 3 .OR.  nOper == 4
		...
		if !QRYZBI->(EOF())
			Help( ,, "MGFCRM27",, "Este vendedor j� � representante de outro Roteiro."+chr(13) + chr(10)+"Roteiro: " + allTrim(QRYZBI->ZBI_CODIGO) + " - " + allTrim(QRYZBI->ZBI_DESCRI), 1, 0 )
			lRet := .F.
		endif
	...
	if nOper == 5
		...
		if !QRYZBJ->(EOF())
			Help( ,, "MGFCRM27_1",, "Este Roteiro j� esta em uso pela Estrutura de Cliente.", 1, 0 )
			lRet := .F.
		endif
...
static function chkDescri(oMdl, cField, xValue, xOldValue)
	...
	if allTrim(cField) == "ZBI_DESCRI"
		if "/" $ xValue
			Help( ,, "MGFCRM27",, "Nao � permitida o carecter / (barra) neste campo.", 1, 0 )
			lRet := .F.
		endif
	endif
