...
Data.....:              30/03/2017
Descricao / Objetivo:   Importacao de vendedores para AO4
Doc. Origem:            GAP CRM
Solicitante:            Cliente
Uso......:              
Obs......:              
...
return paramBox(aParambox, "Importacao de Vendedores"	, @aRet, , , .T. /*lCentered*/, 0, 0, , , .T. /*lCanSave*/, .T. /*lUserSave*/)
...
static function importAO4(cCodSA3, aCodSA1, nLinAtu, nLast)
	...
	if SA1->( DBSeek( xFilial("SA1") + aCodSA1[1] + aCodSA1[2] ) )
		...
		If !lRet
			...
			conout( "MGFCRM09 -> Id do formulario de origem:" + ' [' + AllToChar( aErro[1]  ) + ']' )
			conout( "MGFCRM09 -> Id do campo de origem:     " + ' [' + AllToChar( aErro[2]  ) + ']' )
			conout( "MGFCRM09 -> Id do formulario de erro:  " + ' [' + AllToChar( aErro[3]  ) + ']' )
			...
			conout( "MGFCRM09 -> Mensagem da solucao:       " + ' [' + AllToChar( aErro[7]  ) + ']' )
