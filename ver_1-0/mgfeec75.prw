...
� acionado atraves do programa MGFEEC69
...
fwMsgRun(, { || getDespLan() }	, "VerIficando despesas j� lan�adas"	, "Aguarde. Selecionando historico de lan�amento..." )
...
DEFINE MSDIALOG oDlgDespes TITLE 'Despesas de Despachante : '+Alltrim(cDespachante)+' - '+Alltrim(cNome) FROM aCoors[1], aCoors[2] TO aCoors[3], aCoors[4] PIXEL STYLE DS_MODALFRAME
	...
	oDespesBrw:addColumn({"Valor Pr� Calculo", {||aDespes[oDespesBrw:nAt,nPosValorPre    ]}, "N", pesqPict("ZEE","ZEE_VALOR")	, 2, tamSx3("ZEE_VALOR")[1]		, tamSx3("ZEE_VALOR")[2]	, .F., , .F.,, ,, .F., .T.,, })
	...
	oHistorBrw:addColumn({"Tabela Pr� Calculo"	, {||aDespesLan[oHistorBrw:nAt,04]}, "C", pesqPict("ZEE","ZEE_CODIGO")	, 1, tamSx3("ZEE_CODIGO")[1]/2	,							, .F. , , .F.,, ,, .F., .T.,									, })
	oHistorBrw:addColumn({"Moeda Pr� Calculo"	, {||aDespesLan[oHistorBrw:nAt,05]}, "C", pesqPict("ZEE","ZEE_MOEDA")	, 1, tamSx3("ZEE_MOEDA")[1]/2	,							, .F. , , .F.,, ,, .F., .T.,									, })
	oHistorBrw:addColumn({"Valor Pr� Calculo"	, {||aDespesLan[oHistorBrw:nAt,06]}, "N", pesqPict("ZEE","ZEE_VALOR")	, 2, tamSx3("ZEE_VALOR")[1]		, tamSx3("ZEE_VALOR")[2]	, .F. , , .F.,, ,, .F., .T.,									, })
...
Local aRet			:= {}
Local aParambox		:= {}
Private  lTemPreC	:= .F.  // Tem pr� calculo definido
...
Else    
	...
	If ConPad1(,,,'SYB') //If ConPad1(,,,'XECC73') .AND. SYB->(!EOF())
		If SYB->YB_DESP $ cIncDesp 
			lTemPreC	:= .T.
		Else
			MsgAlert("Despesa nao definida em pr� c�lculo, o pedido ficar� bloqueado !!")
			bPreNota := .T.
		EndIf 
		AAdd( aParambox,{3, "Rateio:"     	,1		 , { "Sim" , "Nao"} , 070 , "" , .T. } )
		AAdd( aParamBox,{1, "Taxa negoc:"   ,0       , "@E 9,999.9999999"	,,, IIF(!lTemPreC , ".F.",'') ,070,.F.})
		...
		If ParamBox(aParambox, "Tipo de Lancamento"	, @aRet, , , .T. /*lCentered*/, 0, 0, , , .F. /*lCanSave*/, .F. /*lUserSave*/)
			...
			IF AScan(aDespesLan ,{|x| Alltrim(x[2]) == Alltrim(SYB->YB_DESP) }) <> 0
				 MsgAlert("Despesa j� lan�ada para uma ou mais Exps, o pedido ficar� bloqueado !!")
				 bPreNota := .T.
			EndIf
...
Local aC7Num		:= {}	// CONTEM OS PEDIDOS USADOS - PARA GERAR GRADE SE NECESS�RIO
Local aAgr          := {}
Local aExpGrv       := {}
Local cQuery        := ''
Local aExclui       := {}
...
If !Empty( aDocsSC7 )
	...
	// Se lana�amento de despesas estiver de acordo com o pre calculo gera Nota de Entrada + Titulo com Grade
	cDoctoAtu := ""
	While !QRYSC7->(EOF())
		aSF1 := {}
	...
	If Len(aNotas) > 0 
		...
		If !Empty( cLogProc2 )
			cLogProc += "Devido a diverg�ncias com o Pr� Calculo, foram geradas as Pr� Notas: " + CRLF + cLogProc2
		EndIf
...
IF nTipo == 1 // Nota fiscal = DOC e Serie
	IF !Empty(MV_PAR01) .AND. !Empty(MV_PAR03) .AND. !Empty(MV_PAR04).AND. !Empty(MV_PAR05) .AND. !Empty(MV_PAR06)
		...
		QRY_VLD->(dbGoTop())
		IF QRY_VLD->(!Eof())
	         MsgAlert('Nota Fiscal/Serie j� lan�ada para o Fornecedor !!')  	
			 bRet  := .F.
		EndIF
	EndIf
