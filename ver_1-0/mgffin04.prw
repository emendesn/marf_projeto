...
Data.....:              15/09/2016
Descricao / Objetivo:   Compensação automática de notas de notas de débito (devolução de compras)
...
If lContabil
	...
	Else
		...
		cLoteCtb := X5Descri()
	EndIf
	If nHdl <= 0 // Tratamento efetuado para não gerar handle quando ja existir um em execução
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡„o    ³FaCmpCP   ³ Autor ³ Eduardo Riera         ³ Data ³11.08.2003 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄ´±±
			...
			If aCompensar[nX][CMP_VLRCMP]<>0
				...
				If RecLock("SE2")
					...
					If nVlBxMd1 <> 0
						...
						If aCompensar[nX][CMP_LCALCM] .and. aTitulo[CR_VLVARMN] != 0    // Geração de Correção monetária
					...
					If nVlBxMd1 <> 0
						...
						If !Empty(SE5->E5_DOCUMEN)
							...
							If aCompensar[nX][CMP_LCALCM] .and. aCompensar[nX][10] != 0    // Geração de Correção monetária
								   	...
								   	If lMaIntDel .and. ExecBlock("MaIntDel",.F.,.F.)
										If AllTrim( SE5->E5_TABORI ) == "FK2"
											...
											If MsSeek( xFilial("FK2") + SE5->E5_IDORIG )
												...
												oModelBxP:SetOperation( MODEL_OPERATION_UPDATE ) //Alteração
												oModelBxP:Activate()
