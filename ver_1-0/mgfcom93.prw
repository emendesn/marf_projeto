...
=====================================================================================
*/
user function MGFCOM93()

	RPCSetType(3)
	RpcSetEnv( '01' , '010001' , Nil, Nil, "COM", Nil )
	SetFunName("MGFCOM93")
...
//cAlias4	:= GetNextAlias()
	...
	cQuery += "	WHERE C7_ZWFPC='S'  " //017013
	cQuery += " AND SC7.D_E_L_E_T_ = ' ' AND SC7.C7_XWFENV='N' "
...
While TEMP1->(!Eof())
		...
		cNum 		:= TEMP1->C7_NUM  //'090623' //
		//cCCO 		:= geteSOLIC() //UsrRetMail(RetCodUsr())
		cFilant     := TEMP1->C7_FILIAL
		...
		If SC7->(MsSeek(xFilial("SC7")+cNum))
			...
			EnviaPC(@aAnexos)
				...
				cPara 		:=  cEmailFor  //'tgaleano@outlook.com' //
			...
			cBody 			:= "Segue em anexo confirma��o do pedido de compra "+cNum+" conforme negocia��o pr�via."
			...
			cCC 		:= getesolic() //+";"+"wander.mota@hotmail.com" //UsrRetMail(RetCodUsr()) //Copia para usuario que enviou o pc para o fornecedor.
			cCCO		:= geteComprad() //"tgaleano@outlook.com" //geteSOLIC()
			cSubject 	:= 'PEDIDO DE COMPRA - No. : ' + SC7->C7_NUM //cNum
			...
			if !lErrEml
				//ApMsgAlert('Pedido enviado ao Fornecedor com Sucesso','E-MAIL')
					...
					//_cQryC7	:= " UPDATE " + RetSqlName("SC7") + " SET C7_XWFENV := 'S' "
					//_cQryC7	+= " WHERE C7_NUM = '"+cNum+"' AND C7_XWFENV <> 'S' "
					//TcSqlExec(_cQryC7)
            else
					dbSelectArea("SC7")
					RecLock('SC7',.F.)
						SC7->C7_XWFENV := 'N' // Workflow enviado
					MsUnlock()
...
Static Function EnviaPC(aAnexos)
	...
	Local npicm        := 0
	Local nI
	Local cDiretorio := "\WORKFLOW\EMP01\TEMP\"
	...
	If oPrinter == Nil
		...
		//cPath 	:= GetPvProfString(GetEnvServer(),"RootPath","ERROR", cIniFile )+'\WORKFLOW\ENV_PC_FORNECEDOR\PC_'+AlltoChar(Alltrim(SC7->C7_NUM))+'\'

		//cPath2 	:= '\WORKFLOW\ENV_PC_FORNECEDOR\PC_'+AlltoChar(Alltrim(SC7->C7_NUM))+'\'
		cPath 	:='\workflow\emp01\temp\"
	...
	nPrintType := 2 
 	cName      := cCdPedido+cFilant
 	//oPrinter     := FWMSPrinter():New(cName, nPrintType,.T.,,.T.,.F.,,"PDF",.T., .T., .F.,.F.)
	oPrinter     := FWMSPrinter():New(cName, nPrintType,.T.,"\WORKFLOW\EMP01\TEMP\",.T.,.F.,,"PDF",.T., .T., .F.,.F.) 
		//oPrinter := FWMSPrinter():New(cName,IMP_PDF,lAdjustToLegacy,,lDisableSetup)
		Conout( "Path Server: "+oPrinter:cPathPrint)
  
		oPrinter:lPDFAsPNG := .F.
		...
		oSetup := FWPrintSetup():New(nFlags,cName)
...
// ----------------------------------------------
	...
	if SC7->(MsSeek( xFilial("SC7") + cCdPedido ))
		...
		oPrinter:Say(nLin-10 , nC_6 	, "Vr. Unit�rio" 		, oFont11B	,, CLR_BLUE)
		...
		While !SC7->(EOF()) .And. xFilial("SC7") == SC7->C7_FILIAL .And. cCdPedido == SC7->C7_NUM
			...
			If MsSeek(xFilial("SB1")+SC7->C7_PRODUTO)
				...
				IF SC7->C7_ZWFPC='S'
                    ...
                    NTOTIPI := NTOTIPI+(SC7->C7_TOTAL*SC7->C7_IPI)/100
                    NTOTICM := NTOTICM+(SC7->C7_TOTAL*SC7->C7_PICM)/100
					nTPC += SC7->C7_TOTAL
	...
	oPrinter:FillRect({nLin ,0050,nLin+(nBor-5),nColFim } , oBrush) //Linha
	nLin += nEsp
	oPrinter:Say(nLin,	nC_1	    ,'Tipo de Frete : '+frx+'  Transportadora         : ' +gettransp()   ,oFont11)
...
//	oPrinter:Say(nLin,	nC_1		, 'NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero do nosso Pedido de Compras.',oFont11B)
		...
		//endif
			...
			nLin += nEsp
			nLin += nEsp
			oPrinter:Say(nLin,	nC_1		, "7. � obrigatorio o uso da transportadora indicada no pedido de compra com o Frete FOB. Caso  nao ocorra, o  valor  do ", oFont11 )
			nLin += nEsp
			oPrinter:Say(nLin,	nC_1		, "	  frete sera revertido para o fornecedor;", oFont11 )
			nLin += nEsp
			nLin += nEsp
			oPrinter:Say(nLin,	nC_1		, "8. O nao cumprimento destas regras acarretar� em bloqueio do pagamento da nota fiscal. Duvidas  entrar em contato com ", oFont11 )
			nLin += nEsp
			...
			nLin += nEsp
			nLin += nEsp
			oPrinter:Say(nLin,	nC_1		, "A cess�o do presente termo � vedada a contratada, sendo permitida apenas se autorizada pela contratante.", oFont11 )
			nLin += nEsp
			nLin += nEsp
			...
			nLin += nEsp
			nLin += nEsp
			oPrinter:Say(nLin,	nC_1		, "Qualquer hipotese de desconto  ou  recebimento  antecipado  dos valores advindos da presente  relacao � vedada, sendo ", oFont11 )
			nLin += nEsp
			oPrinter:Say(nLin,	nC_1		, "permitida apenas se autorizada pela contratante.", oFont11 )
			nLin += nEsp
			nLin += nEsp
			oPrinter:Say(nLin,	nC_1		, "A contratada compromete-se a nao incorrer em qualquer das condutas do artigo 5 da lei anticorrupcao, e que ressarcir� ", oFont11 )
			nLin += nEsp
			...
			nLin += nEsp
			oPrinter:Say(nLin,	nC_1		, "de cada m�s. Documentos emitidos fora  dos  prazos  solicitados e que  nao  forem  entregues  para  registros,  serao ", oFont11 )
			nLin += nEsp
	...
	*/
   	...
   	//aqui para smartclient 
   	//If lPreview
	//	oPrinter:Preview()
	//EndIf
    
    //teste gerar pdf job
	oPrinter:cPathPDF  := "\workflow\emp01\temp\"
	oPrinter:Print() 
    //
    Sleep( 2000 ) //Rafael - 29/11/2018
	*****************************************************
	// Copia arquivos do remote local para o servidor, sem compactar antes de transmitir
	*****************************************************
	//if file(cTempMaq+cCdPedido+".pdf")
		//lCpSrvOk := CpyT2S( cTempMaq+cCdPedido+".pdf", cPath2 , .F. )
	IF File("\workflow\emp01\temp\"+cCdPedido+cFilant+".pdf")
		//lCpSrvOk := __CopyFile("\workflow\emp01\temp\"+cCdPedido+cFilant+".pdf", cPath2+cCdPedido+cFilant+".pdf" )

		//if lCpSrvOk
			aAdd(a_Anexos,cPath2+cCdPedido+cFilant+".pdf")
			//aAdd(aAnexos,cPath2+cCdPedido+cFilant+".pdf")
		//Endif
...
Static Function EnviarMail( cDe, cPara, cCc, cAssunto, aAnexo, cMsg , lExibInfo , cSigaDpto , cCCO , cBody )
	Local lErrEml := .F.
	Local cCtaEml := ''
	Local cSenEml := ''
	...
	If EnvEMail( cDe, cPara, cCc, cAssunto, aAnexo, cMsg , cCCO , cBody )
		//if lExibInfo
...
static function getsolic()
	...
	//Local cUser := UsrFullName(GetAdvFVal('SC1', 'C1_SOLICIT', xFilial( 'SC1' ) + SUBST(SC7->C7_OBS,6,6) , 1, ''))
        ...
        IF SUBST(SC7->C7_OBS,1,2) = "SC"
			...
			ctel  := AllTrim(GetAdvFVal('SAI', 'AI_ZTEL', xFilial( 'SC1' ) + SUBST(SC7->C7_OBS,6,6) , 1, ''))
...
// envmailnova barbi
Static Function EnvEmail(cDe, cPara, cCc, cAssunto, aAnexos, cMsg , cCCO, cBody)
	...
	Local cErrMail
	Local cArqanexo := GETMV("MGF_ANEXO")
	Local cAnexoAux := ''
	Local nLoopAux 	:= 0
	Local lRet 	 	:= .T.
	...
	//IF !File(cArqanexo+"024772010048.pdf")
	//    conout('aquivo nao encontrado')
	//ENDIF
	...
	cArquivo := cAnexo  // cArqanexo+"024772010048.pdf"  //'\workflow\emp01\temp\024772010048.pdf'
