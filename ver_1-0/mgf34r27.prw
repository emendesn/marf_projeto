...
/*/   {Protheus.doc} MGF34R27

Descri??o : Função para efetuar extra??o de dados fiscais a partir de View. 
			Rotina que mostra na tela os dados da planilha: CONTABILIDADE - SFT - Entradas

			Rotina similar a MGF34R17, por?m foi solicitado para separar os fontes, para que possam ser utilizadas 
			chamada de views diferentes, de modo que altera??es do cont?bil e fiscal ocorram separadamente.
...
User Function MGF34R27()
	...
	Aadd(_aDefinePl, "Contabilidade - SFT - Entradas"	)	//01-	_cTitulo	- Titulo da planilha a ser gerada. Aparecerá na regua de processamento.
	Aadd(_aDefinePl, "SFT - Entradas"					)	//02-	_cArqName  - Nome da planilha Excel a ser criada
	Aadd(_aDefinePl, {"SFT - Entradas"}					)	//03-	_cNomAbAna - Titulo(s) da(s) aba(s) na planilha excel
	Aadd(_aDefinePl, {"SFT - Entradas"}					)	//04-	_cNomTTAna - Titulo(s) da(s) tabela(s) na planilha excel
	...
	Aadd(_aCampoQry, {"PRODUTO"		    ,"PRODUTO"   		,"PRODUTO"  		,"C"	,055	,0	,""	,""	,""	})
	...
	Aadd(_aCampoQry, {"DEVOL_ORIG"		,"DEVOL_ORIG"		,"DEVOL_ORIG"		,"N"	,012	,2	,""	,""	,""	})
	Aadd(_aCampoQry, {"NF_ORIGEM"		,"NF_ORIGEM"		,"NF_ORIGEM"		,"C"	,100	,0	,""	,""	,""	})
	...
	Aadd(_aCampoQry, {"COD_BC_CRED"		,"COD_BC_CRED"	,"COD_BC_CRED"	,"C"	,Space(tamSx3("F4_CODBCC")[1])	,0	,""	,""	,""	}) // Henrique Vidal - TOTVS - 20/09/2019 - RTASK0010052
	...
	Aadd(_aCampoQry, {"W6_HAWB"			,"COD_PROCESSO"		,"Cód. Processo"	,"C"	,017	,0	,""	,""	,""	})  
	Aadd(_aCampoQry, {"W6_DI_NUM"		,"NUM_DI"			,"Nº DI"			,"C"	,010	,0	,""	,""	,""	})
	...
	aAdd(_aParambox,{1,"Data Entrada Final"		,Ctod("")						,""		,"U_VLFIMMAI(MV_PAR01, MV_PAR02	,'Data')" 		,""		,"",050,.T.})
	...
	_cQuery += "  FROM " + U_IF_BIMFR( "IF_BIMFR", "V_CONTAB_SFTENTFISCAL"  )       + CRLF
	_cQuery += U_WhereAnd(!empty(_cCODFILIA ),     " FT_FILIAL IN "               + _cCODFILIA                               ) //OBRIGATORIO (SELEÇÃO DO COMBO)  CAMPO FILIAL(06 posições)
	_cQuery += U_WhereAnd(!empty(_aRet[2] ),       " FT_ENTRADA_FILTRO BETWEEN '" + _aRet[1] + "' AND '" + _aRet[2] + "' "   ) // NAO OBRIGATORIO
	_cQuery += U_WhereAnd(!empty(_aRet[4] ),       " FT_CLIEFOR 	   BETWEEN '" + _aRet[3] + "' AND '" + _aRet[4] + "' "   ) // NAO OBRIGATORIO
	_cQuery += U_WhereAnd(!empty(_aRet[6] ),       " FT_TES 		   BETWEEN '" + _aRet[5] + "' AND '" + _aRet[6] + "' "   ) // NAO OBRIGATORIO
	_cQuery += U_WhereAnd(!empty(_aRet[8] ),       " FT_CFOP 		   BETWEEN '" + _aRet[7] + "' AND '" + _aRet[8] + "' "   ) // NAO OBRIGATORIO
	_cQuery += U_WhereAnd(!empty(_aRet[10] ),      " FT_PRODUTO 	   BETWEEN '" + _aRet[9] + "' AND '" + _aRet[10] + "' "  ) // NAO OBRIGATORIO
