...
/*
=====================================================================================
Programa............: MGFFIN06/MULTIBOL
Autor...............: Microsiga 
Data................: 29/11/06
Descrição / Objetivo: IMPRESSAO DO BOLETO BANCARIO COM CODIGO DE BARRAS
Obs.................: Santander,Banco do Brasil,Bradesco,Itau, CEF
=====================================================================================
*/
User Function MULTIBOL()                             
	...
	If Pergunte(cPerg,.T.)
		...
		fwmsgrun(,{|| MGFFIN06T()},"Aguarde...", "Selecionando títulos...")

	Else

		Aviso(OemToAnsi("ATENÇÃO"),OemToAnsi("Rotina cancelada"),{"OK"})
...
Autor...............: Microsiga 
Data................: 29/11/06
Descrição / Objetivo: Abre tela de markbrowse para selecionar titulos
=====================================================================================
*/
Static Function MGFFIN06T()
...
Local	aPergs     := {} 
	...
	cFilter     += "Posicione('SA1',1,xFilial('SA1')+SE1->E1_CLIENTE+SE1->E1_LOJA,'A1_ZBOLETO') != 'N'.And."
	cFilter     += "Posicione('SA1',1,xFilial('SA1')+SE1->E1_CLIENTE+SE1->E1_LOJA,'A1_TIPO') != 'X'"
	...
	MarkBrow ("SE1","E1_OK",,,,cMarca,"fwmsgrun(,{|| U_RF01BMarcaT(.T.)},'Aguarde...','Marcando linhas...')",,,,"U_RF01BMarcaT(.F.)",,,,,,,,.F.)
...
Autor...............: Microsiga 
Data................: 29/11/06
Descrição / Objetivo: Chama processamento
=====================================================================================
*/
...
/*
=====================================================================================
Programa............: RF01BMarcaT
Autor...............: Microsiga 
Data................: 29/11/06
Descrição / Objetivo: Marca todos as linhas
=====================================================================================
*/
...
Autor...............: Microsiga 
Data................: 29/11/06
Descrição / Objetivo: Realiza impressão
Obs.................: Pode ser chamada direto da nota por exemplo, passando os parametros
=====================================================================================
*/
User Function RF01B062(xBanco,xAgencia,xConta,xSubCt,lSetup,cNotIni,cNotFim,cSerie)
	...
	Local nDvnn 	   := 0
	Local nCount := 0
	Local _lVerAntigo  :=  SuperGetMV("MGF_FIN06A",.F.,.F.) 
	...
	While SE1->(!EOF())
		...
		If Empty(SE1->E1_NUMBCO)
			...
			If _lVerAntigo													// Inserido para tentar retirar a sobreposição do Nosso Número
				//atualiza SEE
				DbSelectArea("SEE")
				RecLock("SEE",.f.)
				...
				DbUnlock()
			Else
				Do While .T. 												// Laço que garante que só uma requisição terá acesso à SEE
					If RecLock("SEE",.F.)								
						SEE->EE_FAXATU	:= Soma1(SEE->EE_FAXATU)			// INCREMENTA P/ TODOS OS BANCOS
						cFaixaAtu		:= StrZero(val(SEE->EE_FAXATU),15)	// Incluido por Barbieri
						SEE->( MsUnLock() )
		...
		Else 
			...
			Else
				cFaixaAtu  := StrZero(val(SE1->E1_NUMBCO),15) //Incluido por Barbieri
			Endif	
		Endif
		...
		Else
			...
			If aDadosBanco[1] == "001"
				cNroDoc	:= StrZero(Val(cFaixaAtu),5)
			ElseIf aDadosBanco[1] = "237"
				cNroDoc := StrZero(Val(cFaixaAtu),11)
			ElseIf aDadosBanco[1] == "341"
				cNroDoc := StrZero(Val(cFaixaAtu),8)
			ElseIf aDadosBanco[1] == "033"
				cNroDoc := StrZero(Val(cFaixaAtu),12)
			ElseIf aDadosBanco[1] == "104" //Incluido por Barbieri
				IF aDadosBanco[8] == "1"
		...
		(E1_SALDO-nVlrAbat)					,;//[7]Valor do Titulo
		SE1->E1_VENCTO						,;//[8]Vencimento               // ALTERADO EM 20.09.2010 ERA E1_VENCREA, MAS TAVA DANDO ERRO ERRO DE FATOR DE VENCTO
		AllTrim(SEE->EE_CODEMP)				,;//[9]Convenio
		cNroDoc  								,;//[10]Sequencial
		Iif(SE1->E1_DECRESC > 0,.t.,.f.)		,;//[11]Se tem desconto
	...
	Else
		...
		Aviso(OemToAnsi("ATENÇÃO"),OemToAnsi("Não foram localizados títulos válidos para impressão de boleto"),{"OK"})
...
Autor...............: Microsiga
Data................: 29/11/06
Descrição / Objetivo: Retorna digito de modulo 10
=====================================================================================
*/
...
Autor...............: Microsiga
Data................: 29/11/06
Descrição / Objetivo: Retorna digito de modulo 11
=====================================================================================
*/
...
Autor...............: Microsiga
Data................: 29/11/06
Descrição / Objetivo: Retorna dados de código de barras e nosso numero
=====================================================================================
*/
...
Autor...............: Microsiga
Data................: 29/11/06
Descrição / Objetivo: Ajusta valor da parcela
=====================================================================================
*/
...
Autor...............: Microsiga
Data................: 29/11/06
		Descrição / Objetivo: Impressao do boleto
=====================================================================================
*/
Static Function Impres2(oPrint,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,aCB_RN_NN)
	...
	If aDadosBanco[1] != "104"
		...
		Elseif aDadosBanco[1] == "001"										// Incluido por Barbieri
	...
	If aDadosBanco[1] == "341" .or. aDadosBanco[1] == "748" .or. aDadosBanco[1] == "237" //.or. aDadosBanco[1] == "104" // Incluido por Barbieri 
		...
		oPrint:Say  (nRow2+0850,100 ,Subs(SEE->EE_ZCNPJ,1,2)+"."+Subs(SEE->EE_ZCNPJ,3,3)+"."+Subs(SEE->EE_ZCNPJ,6,3)+"/"+Subs(SEE->EE_ZCNPJ,9,4)+"-"+;
		Subs(SEE->EE_ZCNPJ,13,2)+"-"+alltrim(SEE->EE_ZUNID),oFont10) 
	...
	Elseif aDadosBanco[1] == "341" //TRATAMENTO ITAU
		oPrint:Say  (nRow3+2140,100 ,Subs(SEE->EE_ZCNPJ,1,2)+"."+Subs(SEE->EE_ZCNPJ,3,3)+"."+Subs(SEE->EE_ZCNPJ,6,3)+"/"+Subs(SEE->EE_ZCNPJ,9,4)+"-"+;
		Subs(SEE->EE_ZCNPJ,13,2)+"-"+alltrim(SEE->EE_ZUNID),oFont10) 
	Else
	...
	If aDadosBanco[1] == '237'
		oPrint:Say  (nRow3+2300,555 ,substr(aDadosBanco[6],2,2),oFont10)
	ElseIf aDadosBanco[1] == '104'
...
Autor...............: Microsiga
Data................: 29/11/06
Descrição / Objetivo: Cria box no tmsprinter
=====================================================================================
*/
...
=====================================================================================
Programa............: sMod11
Autor...............: Wanderley Monteiro
Data................: 01/08/02
Descrição / Objetivo: Calcula o digito verificador com Modelo 11 
=====================================================================================
*/
...
Autor...............: Microsiga
Data................: 01/08/02
Descrição / Objetivo: Verifica informacoes de banco
=====================================================================================
*/
...
Autor...............: Microsiga
Data................: 01/08/02
Descrição / Objetivo: Cria tabela temporaria com Select via query
=====================================================================================
*/
...
Autor...............: Microsiga
Data................: 01/08/02
Descrição / Objetivo: Retorna digito do modulo 6
=====================================================================================
*/
