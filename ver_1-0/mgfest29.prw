...
Data................: 09/06/2017
Descricao / Objetivo: Rotina temporaria para limpar o empenho do produto. OBS: Deletar do RPO depois de rodado o refaz empenho.
Doc. Origem.........: 
Solicitante.........: Cliente
Uso.................: 
Obs.................: 
...
Define MsDialog oDlg Title "Acerto de Empenho SB2 ( ***ROTINA TEMPOR�RIA*** )" From 116,090 To 300,630 Pixel
...
If nReg == 0
	MsgAlert("Nao h� Empenhos para exibir neste filtro.")
	Return()
Endif	
...
If SB1->(dbSeek(xFilial("SB1")+(cAliasTrb)->B2_COD))
	...
	If !lContinua	
		(cAliasTrb)->(dbCloseArea())
		MsgAlert("Produto com controle de Lote ou Endereco habilitado. Nao sera possivel zerar o empenho por esta rotina.")
		Return()
	Endif
Endif
...
If (cAliasTrb)->(B2_QEMP) <= nQuantSD4
	(cAliasTrb)->(dbCloseArea())
	MsgAlert("Empenho do produto � devido as OP�s em aberto."+CRLF+;
	"Nao sera possivel excluir o empenho por esta rotina.")
	Return()
Elseif nQuantSD4 > 0
	MsgInfo("Empenho nao sera totalmente zerado, pois h� empenhos de OP�s"+CRLF+;
	"Total de Empenhos de OP: "+Alltrim(Str(nQuantSD4)))	
Endif
		...
		While (cAliasMark)->(!Eof())
			...
			//If IsMark( "B2_MSEXP" , cMarca , lInverte )
				...
				If SB2->(Recno()) == (cAliasMark)->RECNO
					ProcSB2(cAliasMark,nQuantSD4,lContinua)
				Endif	
			//Endif	
			(cAliasMark)->(dbSkip())
		EndDo
...
Static Function ProcSB2(cAliasMark,nQuantSD4,lContinua)

SB2->(RecLock("SB2",.F.))
//If lContinua
	SB2->B2_QEMP := nQuantSD4
//Endif	
//SB2->B2_MSEXP := "" // limpa campo usado na marca
SB2->(MsUnLock())        
...
cQ += "AND D4_COD = '"+mv_par01+"' "
cQ += "AND D4_LOCAL = '"+mv_par02+"' "
cQ += "AND D4_SITUACA = ' ' "
cQ += "AND D4_QUANT > 0 "
