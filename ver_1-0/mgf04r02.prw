...
//������������������������������������������������������������������������������������������������������������������������������������������������͹��
//���Desc.		� Rotina que mostra na tela os dados da planilha: ESTOQUE - Acompanhamento de Recebimento	(Modulo 04-ESTOQUE)					���
...
User Function MGF04R02()
	...
	Aadd(_aDefinePl, "ESTOQUE - Acompanhamento de Recebimento"	)	//01-  _cTitulo	- Titulo da planilha a ser gerada. Aparecera na regua de processamento.
	...
	Aadd(_aCampoQry, {	"XXF1_TIPO"		,"TIPO_OPERACAO_NF		as TIPOOPERNF"	,"Tipo de Opercacao da NF"	,"C"	,0040,   0		,""		,""	,""	})
	Aadd(_aCampoQry, {	"ZZH_AR"		,"NUM_AR"								,"N� AR"					,""		,""		,""		,""		,""	,""	})
	Aadd(_aCampoQry, {	"F1_DTLANC"		,"DT_LANCAMENTO_NF		as DTLANCAMNF"	,"Data lancamento NF"		,""		,""		,"" 	,""		,""	,""	})
	Aadd(_aCampoQry, {	"ZZH_DOC"		,"NUMERO_NF"							,"N� NF"					,""		,""		,"" 	,""		,""	,""	})
	Aadd(_aCampoQry, {	"ZZH_SERIE"		,"SERIE_NF"								,"Serie NF"					,""		,""		,"" 	,""		,""	,""	})
	Aadd(_aCampoQry, {	"F2_DOC"		,"NF_ORIGEM"							,"NF Origem"				,""		,""		,"" 	,""		,""	,""	})
	Aadd(_aCampoQry, {	"F2_SERIE"		,"SERIE_NF_ORIGEM		as SERIENFORI"	,"Serie NF Orig"			,""		,""		,"" 	,""		,""	,""	})
	Aadd(_aCampoQry, {	"F2_EMISSAO"	,"EMISSAO_NF_ORIGEM		as EMISSNFORI"	,"Emissao NF Ori"			,""		,""		,"" 	,""		,""	,""	})
	Aadd(_aCampoQry, {	"D1_ZRAMI"		,"NUM_RAMI"								,"Cod.Rami"					,""		,""		,"" 	,""		,""	,""	})
	...
	Aadd(_aCampoQry, {	"ZZI_LOCAL"		,"COD_LOCAL_ARMAZ		as CODLOCALAR"	,"Cod. Local Armazem"		,""		,""		,"" 	,""		,""	,""	})
	...
	Aadd(_aCampoQry, {	"ZZI_QDEV"		,"QTD_DEVOLUCAO			as QTDDEVOLUC"	,"Qtd. Devolucao"			,""		,""		,"" 	,""		,""	,""	})
	...
	Aadd(_aCampoQry, {	"ZZI_SERIE"		,"NF_ACERTO_SERIE		as SERIE_ACER"	,"Serie NF Acerto"			,""		,""		,"" 	,""		,""	,""	})
	...
	Aadd(_aCampoQry, {	"B14_MOTIVO"	,"OBS_AR"								,"Observacao AR"			,""		,""		,"" 	,""		,""	,""	})
	Aadd(_aCampoQry, {	"ABS_DESCRI"	,"STATUS_AR"							,"Status AR"				,""		,""		,"" 	,""		,""	,""	})

	aAdd(_aParambox,{1,"Dt lancamento NF Inicial"	,Ctod("")						,""		,""													,""			,"",050,.F.})
	aAdd(_aParambox,{1,"Dt lancamento NF Final"		,Ctod("")						,""		,"U_VLFIMMAI(MV_PAR01, MV_PAR02,'Dt lancamento')"	,""			,"",050,.T.})
	...
	If _aRet[1] > _aRet[2]
		MsgStop("A Data de lancamento Inicial, nao pode ser maior que a data de lancamento Final.")
		Return.F.
	Endif
	...
	cQTipOpeNF += " union all select  'D' as CAMPO_01, 'DOCTO. DE DEVOLUCAO'					as CAMPO_02 from dual
	cQTipOpeNF += " union all select  'B' as CAMPO_01, 'DOCTO. DE BENEFICIAMENTO'				as CAMPO_02 from dual
	aCpoTipOpe	:=	{{ "CAMPO_01"	,"Tipo"						,01	} ,;
					 { "CAMPO_02"	,"Descricao do Tipo de NF"	,40	} } 
	cTitTipOpe	:= "Marque os Tipos de operacoes de NF � serem listadas: "
	...
	If empty(_aRet[1])  // se for Vazio
		_cQuery += U_WhereAnd( !empty(_aRet[2] ),   " DT_LANCAMENTO_NF_FILTRO BETWEEN '  '" + " AND '" + _aRet[2] + "' "              ) // OBRIGATORIO - SEM VALIDACAO DE DATA E PRIMEIRA DATA PODE SER EM BRANCO DEVIDO AR SEM NF(SINISTRO)
	Elseif ( !empty(_aRet[1]) .and. !empty(_aRet[2]) )  // se for NaoVazio
		_cQuery += U_WhereAnd( !empty(_aRet[2] ),   " DT_LANCAMENTO_NF_FILTRO BETWEEN '"    + _aRet[1] + "' AND '" + _aRet[2] + "' "  ) // OBRIGATORIO - SEM VALIDACAO DE DATA E PRIMEIRA DATA PODE SER EM BRANCO DEVIDO AR SEM NF(SINISTRO)
	Endif
	_cQuery += U_WhereAnd( !empty(_aRet[3] ),   " NUM_AR LIKE '%"              + _aRet[3]   + "%' "                        )	// NAO OBRIGATORIO (USUARIO DIGITA)
	_cQuery += U_WhereAnd( !empty(_aRet[4] ),   " NUMERO_NF LIKE '%"           + _aRet[4]   + "%' "                        )	// NAO OBRIGATORIO (USUARIO DIGITA)
	_cQuery += U_WhereAnd( !empty(_aRet[5] ),   " SERIE_NF = '"                + _aRet[5]   + "' "                         )	// NAO OBRIGATORIO (USUARIO DIGITA)
	_cQuery += U_WhereAnd( !empty(_aRet[8] ),   " COD_FORNECEDOR BETWEEN '"    + _aRet[6]   + "' AND '" + _aRet[8]  + "' " )	// NAO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[9] ),   " LOJA_FORNECEDOR BETWEEN '"   + _aRet[7]   + "' AND '" + _aRet[9]  + "' " )	// NAO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[11] ),  " COD_PRODUTO BETWEEN '"       + _aRet[10]  + "' AND '" + _aRet[11] + "' " )	// NAO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(cTipoOPeNF),  " TIPO_OPERACAO_NF_FILTRO IN " + cTipoOPeNF	                               )	// NAO OBRIGATORIO (COMBO PARA SELECIONAR - SELECT PARA POPULAR O COMBO ABAIXO)
