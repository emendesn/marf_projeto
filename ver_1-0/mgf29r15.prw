...
//ฑฑบPrograma  ณ MGF29R15	บAutor  ณ Geronimo Benedito Alves																	บData ณ08/01/18	บฑฑ
...
User Function MGF29R15()
	...
	Aadd(_aDefinePl, "EXPORTAวรO - Controle AWB x ETA"	)	//01-	_cTitulo	- Titulo da planilha a ser gerada. Aparecerแ na regua de processamento.
	...
	Aadd(_aCampoQry, {"EEC_ZEXP"	,"NUM_EXP"								,"Nบ EXP"				,"C",013	,0, "" 				,	,})
	Aadd(_aCampoQry, {"EEC_ZANOEX"	,"ANO_EXP"								,"Ano EXP"				,"C",002	,0, "" 				,	,})
	Aadd(_aCampoQry, {"EEC_ZSUBEX"	,"SUBEXP"								,"SUBEXP"				,"C",003	,0, "" 				,	,})
	Aadd(_aCampoQry, {"A1_COD"		,"CODIGO_TRADING 		as COD_TRADIN"	,"C๓d. Trading"			,"C",006	,0, "" 				,	,})
	Aadd(_aCampoQry, {"A1_NOME"		,"NOME_TRADING 			as NOM_TRADIN"	,"Nome Trading"			,"C",040	,0, ""				,	,})
	Aadd(_aCampoQry, {"A1_COD"		,"CODIGO_IMPORTADOR 	as COD_IMPORT"	,"C๓d. Importador"		,"C",006	,0, "" 				,	,})
	...
	Aadd(_aCampoQry, {"EX9_CONTNR"	,"NUM_CONTAINER 		as NCONTAINER"	,"Nบ Container"			,"C",020	,0, "" 				,	,})
	...
	_cQuery += U_WhereAnd( !empty(_aRet[2] ),    " DATA_BL_FILTRO  BETWEEN '"   + _aRet[1] + "' AND '" + _aRet[2] + "' " ) //OBRIGATORIO, COM A VALIDAวรO DE 35 DIAS
	_cQuery += U_WhereAnd( !empty(_cCODFILIA ),  " FILIAL_FILTRO IN "           + _cCODFILIA                             ) //OBRIGATORIO (SELEวรO DO COMBO)  CAMPO FILIAL(06 posi็๕es)
	_cQuery += U_WhereAnd( !empty(_aRet[4] ),    " CODIGO_TRADING BETWEEN '"    + _aRet[3] + "' AND '" + _aRet[4] + "' " ) //NรO ษ OBRIGATORIO (CODIGO DE ATE - TABELAS SA1010)
	_cQuery += U_WhereAnd( !empty(_aRet[6] ),    " CODIGO_IMPORTADOR BETWEEN '" + _aRet[5] + "' AND '" + _aRet[6] + "' " ) //NรO ษ OBRIGATORIO (CODIGO DE ATE - TABELAS SA1010)
	_cQuery += U_WhereAnd( !empty(_aRet[8] ),    " DATA_AWB_FILTRO	 BETWEEN '" + _aRet[7] + "' AND '" + _aRet[8] + "' " ) //NรO ษ OBRIGATORIO (CODIGO DE ATE - TABELAS SA1010)
