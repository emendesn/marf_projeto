...
Data.....:              09/11/2016                                                                                                            
Descricao / Objetivo:   Relatório de Aviso de Recebimento                        
Doc. Origem:            Contrato - GAP MGFEST07
Solicitante:            Cliente
Uso......:              Marfrig
Obs......:              Relatório de Aviso de Recebimento Específico
=====================================================================================
*/
...
±±ºPrograma  ³IMPREL    ºAutor  ³ A.Carlos     o     º Data ³  09/11/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Relatorio de pedido de compra             		          º±±
±±º          ³                                                            º±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³CabecMarf ºAutor  ³ A.Carlos           º Data ³  09/11/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
If dbSeek(cEmpAnt + TRBAR->F1_FILIAL)
	...
	oPrn:Line (_nLin,100,_nLin,2300)
	_nLin+=050
	oPrn:Say( _nLin, 0300,'Num. NF: '+ALLTRIM(TRBAR->F1_DOC)+"       Série: "+ALLTRIM(TRBAR->F1_SERIE) , oFontC1 )
	_nLin+=050
	oPrn:Say( _nLin, 0300,'Data Emissão: '+Substr(TRBAR->F1_EMISSAO,7,2)+"/"+Substr(TRBAR->F1_EMISSAO,5,2)+"/"+Substr(TRBAR->F1_EMISSAO,1,4) , oFontC1 )
	...
	//oPrn:Say( _nLin, 2000,"Conhecimento: "+TRBAR->DB1_ZCONHE , oFont3 ) //+"   Série: "
	_nLin+=050
	oPrn:Say( _nLin, 0300,"Usuário: "+ __cUserId + ' ' + cUserName , oFont3 )
	oPrn:Say( _nLin, 1500,'Data Inclusão: '+Substr(TRBAR->F1_DTDIGIT,7,2)+"/"+Substr(TRBAR->F1_DTDIGIT,5,2)+"/"+Substr(TRBAR->F1_DTDIGIT,1,4) , oFont3 )
...
//      _nQtde := Str(TRBAR->DB3_QUANT) 
		...
		// inclusão da descrição complementar
		nLinhas := MLCount(Posicione("SB1",1,xFilial("SB1")+TRBAR->D1_COD,"B1_ZPRODES"),70)

		If !Empty(Posicione("SB1",1,xFilial("SB1")+TRBAR->D1_COD,"B1_ZPRODES"))
			For nXi := 1 To nLinhas
        		cTxtLin := MemoLine(Posicione("SB1",1,xFilial("SB1")+TRBAR->D1_COD,"B1_ZPRODES"),70,nXi)
				If !Empty(cTxtLin)
					If nXi == 1
				    	oPrn:Say(_nLin+=50,100,("Complemento : "+AllTrim(cTxtLin)),oFonte02)
					Else
						oPrn:Say(_nLin+=50,350,AllTrim(cTxtLin),oFonte02)
					EndIf
        		EndIf
			Next nXi
		EndIf	
		...
		IF !Empty(TRBAR->B5_CEME )
			_nLin+=050	
			oPrn:Say( _nLin, 100,'Especificação : '+TRBAR->B5_CEME , oFonte02 ) 		 		
		EndIF

		cOBS := Posicione("SC1",1,xFilial("SC1")+TRBAR->C7_NUMSC+TRBAR->C7_ITEMSC,"C1_OBS")
		IF !Empty(cOBS )
			_nLin+=050				  
			oPrn:Say( _nLin, 0100,'Observação Solicitação : '+cOBS, oFonte02 ) 
        EndIF                                          
        
		cOBS := TRBAR->C7_OBS		
        IF !Empty(cOBS )
			_nLin+=050				  
			oPrn:Say( _nLin, 0100,'Observação Pedido : '+cOBS, oFonte02 ) 
        EndIF                                                  
...
_nLin+=100
oPrn:Say( _nLin, 0400,"Ass. Recepção NF: " , oFont3 )	    	
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ImpComp   ºAutor  ³ A.Carlos           º Data ³  10/11/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Imprime Compradores QUERY PARA ACHAR NOME COMPRADORES       º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ Marfrig	                                                  º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
Static Function ValidPerg( _cPerg )
	...
	aAdd(aPerg,{_cPerg,"07","Documento Até ?","mv_ch7","C",09,0,1,"G","","mv_par07","","","","","","","","","","","","","","","SF1ZZZ",})
	aAdd(aPerg,{_cPerg,"08","Serie Até ?"		,"mv_ch8","C",03,0,1,"G","","mv_par08","","","","","","","","","","","","","","","",})
	...
	aAdd(aPerg,{_cPerg,"11","Fornecedor Até ?"	,"mv_ch11","C",tamSx3("A2_COD")[1]	,0,1,"G","","mv_par11","","","","","","","","","","","","","","","SA2A",})
	aAdd(aPerg,{_cPerg,"12","Loja Até ?"		,"mv_ch12","C",tamSx3("A2_LOJA")[1]	,0,1,"G","","mv_par12","","","","","","","","","","","","","","","",})
...
// cria opcao no menu da rotina de pre-nota de entrada, para impressao do relatório de recebimento de aviso de recebimento 
