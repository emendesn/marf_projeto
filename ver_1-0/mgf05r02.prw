...
//±±ºPrograma  ³ MGF05R02	ºAutor  ³ Geronimo Benedito Alves																	ºData ³  24/04/18  º±±
...
//±±ºDesc.		³ Rotina que mostra na tela os dados da planilha: FATURAMENTO - Relatório de Devolução					(Módulo 05-FAT)			   º±±
//±±º			³ Os dados sao obtidos e mostrados na tela atraves da execução de query, e depois, o usuario pode gerar uma planilha excel com elesº±±
...
User Function MGF05R02()
	...
	Aadd(_aDefinePl, "Faturamento - Relatório de Devolução"	)	//01-  _cTitulo	- Titulo da planilha a ser gerada. Aparecerá na regua de processamento.
	Aadd(_aDefinePl, "Relatório de Devolução"				)	//02-  _cArqName  - Nome da planilha Excel a ser criada
	Aadd(_aDefinePl, {"Relatório de Devolução"}				)	//03-  _cNomAbAna - Titulo(s) da(s) aba(s) na planilha excel
	Aadd(_aDefinePl, {"Relatório de Devolução"}				)	//04-  _cNomTTAna - Titulo(s) da(s) tabela(s) na planilha excel
	...
	Aadd(_aCampoQry, {"F2_FILIAL"	,"EMPRESA"								,"Cód. Empresa"			,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"A1_NOME"		,"NM_EMPRESA"							,"Nome Empresa"			,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"F1_TIPO"		,"TIPO_PED"								,"Tipo Pedido"			,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"F1_DOC"		,"N_NF"									,"Nº Nota Fiscal"		,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"F1_SERIE"	,"SERIE_NF"								,"Serie NF"				,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"F1_FORNECE"	,"COD_CLIENTE			as COD_CLIENT", "Cód. Cliente"			,""		,	, 	,""		,""	,""	})
	...
	Aadd(_aCampoQry, {"ZAV_CODIGO"	,"N_RAMI"								,"Nº RAMI"				,""		,	, 	,""		,""	,""	})
	...
	Aadd(_aCampoQry, {"F1_DTDIGIT"	,"DT_DIGITACAO			as DT_DIGITAC"	,"Data Digitação"		,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"F1_DTLANC"	,"DT_LANCAMENTO			as DT_LANCAME"	,"Data Lançamento"		,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"D1_COD"		,"COD_SKU"								,"Cód. SKU"				,""		,	, 	,""		,""	,""	})
	AAdd(_aCampoQry, {"B1_DESC"		,"NM_SKU"								,"Nome SKU"				,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"D1_QUANT"	,"QTDE_DEVOLV 			as QTDEDEVOLV"	,"Qtde Devolvida"		,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"D1_VUNIT"	,"VLR_UNIT"								,"Vlr. Unitário"		,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"D1_TOTAL"	,"VLR_TOTAL"							,"Vlr. Total"			,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"D1_NFORI"	,"N_NF_ORIGEM 			as NF_ORIGEM"	,"Nº NF Origem"			,""		,	, 	,""		,""	,""	})
	AAdd(_aCampoQry, {"D1_SERIE"	,"SERIE_NF_ORIGEM		as SERIE_ORIG"	,"Série Origem"			,""		,	, 	,""		,""	,""	})
	AAdd(_aCampoQry, {"D2_PEDIDO"	,"N_PEDIDO_ORIGEM		as PEDIDO_ORI"	,"Nº Pedido Origem"		,""		,	, 	,""		,""	,""	}) 
	Aadd(_aCampoQry, {"D1_ITEMORI"	,"SKU_ORIGEM"							,"SKU Origem", 			""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"C5_VEND1"	,"COD_VENDEDOR			as COD_VENDED"	,"Cód. Vendedor", 		""		,	, 	,""		,""	,""	})
	...
	Aadd(_aCampoQry, {"ZBG_REPRES"	,"COD_GERENTE			as COD_GERENT"	,"Cód. Gerente"			,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"A3_NOME"		,"NM_GERENTE"							,"Nome Gerente"			,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"DAK_TRANSP"	,"COD_TRASPORTADOR		as CODTRANSPO"	,"Cód. Transportadora"	,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"A4_NOME"		,"NM_TRANSPORTADOR		as NOMTRANSPO"	,"Nome Transportadora"	,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"DAK_SEQCAR"	,"N_OE"									,"Nº OE"				,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"DA3_PLACA"	,"N_PLACA"								,"Placa do Veículo"		,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"C5_ZIDEND"	,"COD_REGIAO"							,"Cód. da Região"		,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"ZP_DESCREG"	,"NM_REGIAO"							,"Nome da Região"		,""		,	, 	,""		,""	,""	})
	Aadd(_aCampoQry, {"ZBD_DESCRI"	,"DIRETORIA"							,"Diretoria"			,""		,	, 	,""		,""	,""	})

	aAdd(_aParambox,{1,"Data Emissão Inicial"	,Ctod("")						,""	,""													,""		,""	,050,.F.})
	aAdd(_aParambox,{1,"Data Emissão Final"		,Ctod("")						,""	,"U_VLFIMMAI(MV_PAR01, MV_PAR02, 'Data Emissão')"	,""		,""	,050,.F.})
	aAdd(_aParambox,{1,"Data Digitação Inicial"	,Ctod("")						,""	,""													,""		,""	,050,.F.})
	aAdd(_aParambox,{1,"Data Digitação Final"	,Ctod("")						,""	,"U_VLFIMMAI(MV_PAR03, MV_PAR04, 'Data Digitação')"	,""		,""	,050,.F.})
	...
	Endif
	If _aRet[3] > _aRet[4]
		MsgStop("A Data de Digitação Inicial, não pode ser maior que a data de Digitação Final.")
		Return.F.
	Endif
//===		S E L E C I O N A	M O T I V O
	cQryMotivo	:= " SELECT '" +SPACE(TamSx3("ZAU_MOTIVO")[1])+ "' AS ZAS_DESCRI, '" +SPACE(TamSx3("ZAS_CODIGO")[1])+ "' AS ZAS_CODIGO, 'Não Informado' as OBSERVACAO FROM DUAL UNION ALL "
	...
	cTituMotiv	:= "Marque opção do Motivo a serem listado: "
	...
	_cQuery += U_WhereAnd( !empty(_aRet[6] ),    " COD_CLIENTE BETWEEN '"         + _aRet[5]  + "' AND '" + _aRet[6]  + "' " )	// NÃO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[8] ),    " COD_SKU BETWEEN '"             + _aRet[7]  + "' AND '" + _aRet[8]  + "' " )	// NÃO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[10] ),   " COD_GERENTE BETWEEN '"         + _aRet[9]  + "' AND '" + _aRet[10] + "' " )	// NÃO OBRIGATORIO
	_cQuery += U_WhereAnd( !empty(_aRet[12] ),   " COD_VENDEDOR BETWEEN '"        + _aRet[11] + "' AND '" + _aRet[12] + "' " )	// NÃO OBRIGATORIO
	If empty(cMotivoPro)
