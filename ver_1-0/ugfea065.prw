...
Static Function MenuDef()
	...
	Aadd( aRotina, { "Dados da Confer�ncia", "GFE66AP(.F.)", 0, 4, 0,,, })
...
Static Function ViewDef()
	...
	If lCpoTES
		...
		oStructGW3:AddGroup("GrpCsg", "Consignat�rio", "5", 2)
	...
	Else
		...
		oStructGW3:AddGroup("GrpCsg", "Consignat�rio", "4", 2)
		oStructGW3:AddGroup("GrpDFO", "Documento de Frete de Origem", "4", 2)
		oStructGW3:AddGroup("GrpInt", "Integra��es", "4", 2)
	EndIf
...
User Function UGFEAVP(oModel,oViewImp)
	...
	If nOpc == 3 .Or.  nOpc == 4
		...
		If !empty(cChvCte) .And.  lEspCTe .And.  lEmiCTe
			If u_GFE065VLDV(cChvCte) .and.  Empty(cTpCte) .and.  !IsInCallStack("GFEA115PRO")
				oModel:SetErrorMessage(,,,,,"Tipo do CT-e em branco.","� necessario preencher o tipo do CT-e.")
				Return .F.
			EndIf
		EndIf

		If oModelGW3:GetValue("GW3_TPDF") <> "3" .And.  oModelGW3:GetValue("GW3_VLDF") == 0
			Help( ,, "HELP",, "Nao � permitido criar um documento de frete deste tipo com valor zerado.", 1, 0)
			Return .F.
		EndIf
	...
	If nOpc == 3
		...
		If AScan(aGVTStruct, {|x| x[1] == "GVT_FORMNM"}) <> 0
			...
			If GVT->( dbSeek(xFilial("GVT") + oModelGW3:GetValue("GW3_CDESP")) )
				...
				If  lAspaInic .Or.  lAspaFim
					If GVT->GVT_FORMNM == "1"
						oModel:SetErrorMessage(,,,,,"Numero do Documento de Frete com algarismos em formato diferente do parametrizado no cadastro de Especie.","Informe um numero com algarismos em formato compat�vel com o parametrizado no cadastro de Especie do Documento de Frete.")
						Return .F.
					ElseIf GVT->GVT_FORMNM == "2"
						oModel:SetErrorMessage(,,,,,"Numero do Documento de Frete com algarismos ou letras em formato diferente do parametrizado no cadastro de Especie.","Informe um numero com algarismos ou letras em formato compat�vel com o parametrizado no cadastro de Especie do Documento de Frete.")
						Return .F.
		...
		For nI := 1 To Len(aFil)
			...
			If GW3->( dbSeek(aFil[nI][1] + oModelGW3:GetValue("GW3_CDESP") + oModelGW3:GetValue("GW3_EMISDF") + cChaveDF) )
				...
				oModel:SetErrorMessage(,,,,,"J� existe Documento de Frete cadastrado com a chave informada na Filial " + Iif(aFil[nI][1] == cFilAnt, "corrente", aFil[nI][1] + " - " + aFil[nI][2]),"Verifique se os dados informados estao corretos.")
	...
	If !IsInCallStack("U_GFE65In")
		...
		If nOpc <> 5
			...
			If FwFldGet("GW3_TPDF") <> "3"
				...
				If nBasImp > 0 .And.  nBasImp <> nVlDf .And.  !(cTrbImp $ "53") .And.  Abs(nBasImp - nVlDf) <> nPedag
					oModel:SetErrorMessage(,,,,,"Documentos com Base de Imposto menor que o valor do documento devem ter tipo de tributa��o Reduzido","Informe uma Base de Imposto maior ou altere o tipo de tributa��o do documento.")
					Return .F.
				EndIf
			EndIf
			...
			If !Empty(FwFldGet("GW3_CDCONS"))
				If Posicione("GU3", 1, xFilial("GU3") + FwFldGet("GW3_CDCONS"), "GU3_EMFIL") == "2"
					oModel:SetErrorMessage(,,,,,"O consignat�rio do frete nao � Filial.","O consignat�rio deve ser Filial")
					Return .F.
				EndIf
			EndIf
			...
			If FwFldGet("GW3_TRBIMP") == "2"
				If FwFldGet("GW3_VLIMP") <> 0.OR.FwFldGet("GW3_BASIMP") <> 0
					oModel:SetErrorMessage(,,,,,"Valor e Base de Imposto maiores que zero.","Quando a tributa��o informada for 'Isento/Nao-tributado', o Valor e a Base de Imposto devem ser iguais a zero.")
					Return .F.
				EndIf
			ElseIf FwFldGet("GW3_TRBIMP") == "6"
				If FwFldGet("GW3_BASIMP") == 0
					oModel:SetErrorMessage(,,,,,"Base de Imposto nao informada.","Quando a tributa��o for 'Outros', a Base de Imposto deve ser informada.")
					Return .F.
				EndIf
			Else
				If FwFldGet("GW3_VLIMP") == 0 .Or.  FwFldGet("GW3_BASIMP") == 0 .Or.  FwFldGet("GW3_PCIMP") == 0
					oModel:SetErrorMessage(,,,,,"Valor, Base e Aliquota de Imposto nao informados.","Quando a tributa��o for 'Tributado', 'Substituicao Tributaria', 'Diferido' ou 'Presumido', o Valor, a Base e a Aliquota do Imposto devem ser informados.")
					Return .F.
				EndIf
			EndIf
		...
		If (nOpc == 3 .Or.  nOpc == 4)
				...
				If GVT->GVT_TPIMP == "1"
					...
					If !aRetVldCte[1]
						If !(Empty(oModelGW3:GetValue("GW3_CTE"))) .and.  (GVT->GVT_CHVCTE == "2") .And.  Posicione("GU3", 1, xFilial("GU3")+oModelGW3:GetValue("GW3_EMISDF"), "GU3_CTE") == "1"
							oModel:SetErrorMessage(,,,,,"Chave Ct-e inv�lida:" + Chr(13)+Chr(10) + aRetVldCte[2],"Informe uma Chave de CT-e valida.")
							Return .F.
			...
			If cParamRat == "5"
				GFEMsgErro(If( cPaisLoc $ "ANG|PTG", "Parametro MV_CRIRAT (Crit�rio de Rateio) nao esta registado", "Parametro MV_CRIRAT (Crit�rio de Rateio) nao esta cadastrado" ))
			EndIf
			...
			For nCont := 1 To oModelGW4:GetQtdLine()
				...
				If !(oModelGW4:IsDeleted(nCont))
					...
					If IsBlind()
							...
							While !Eof() .And.  GWU->GWU_FILIAL+GWU->GWU_CDTPDC+GWU->GWU_EMISDC+GWU->GWU_SERDC+GWU->GWU_NRDC == GW1->GW1_FILIAL+GW1->GW1_CDTPDC+GW1->GW1_EMISDC+GW1->GW1_SERDC+GW1->GW1_NRDC
								...
								If GU3->(DbSeek(xFilial("GU3")+oModelGW3:getValue("GW3_EMISDF"))) .And.  GV5->(DbSeek(xFilial("GV5")+GW1->GW1_CDTPDC))
									If GWU->GWU_CDTRP == oModelGW3:getValue("GW3_EMISDF") .And.  Empty(GWU->GWU_DTENT) .And.  GU3->GU3_ENTOBR == "1" .And.  GV5->GV5_SENTID $ "23"
										oModel:SetErrorMessage(,,,,,"Tentativa de vincular um Documento de Carga sem entrega para esse Emissor.","� necessario realizar a entrega do Documento de Carga: "+Chr(13)+Chr(10)+Chr(13)+Chr(10)+"- Serie: " +alltrim(GW1->GW1_SERDC) + " Documento de carga: "+alltrim(GW1->GW1_NRDC)+".")
										Return .F.
									EndIf
									lTrechoEmi := .T.
								EndIf
						...
						If !lTrechoEmi
							oModel:SetErrorMessage(,,,,,"Tentativa de inclusao de Documentos de Carga sem trecho atendido pelo Emissor.","Verifique se existem c�lculos vinculados ao Documento de Carga.")
							Return .F.
						EndIf
						If lTrechoEmi
...
User Function UGFE5VL(oModel)
	...
	If (nOpc == 4 .OR.  nOpc == 5 ) .And.  !ISINCALLSTACK("u_GFE65In")
		...
		If GW3->GW3_SITFIS == "5"
			Help( ,, "HELP",, "O Documento de Frete esta Pendente de Desatualiza��o no Fiscal, � necessario aguardar retorno.", 1, 0)
			Return .F.
		EndIf
		...
		If GW3->GW3_SITREC == "5"
			Help( ,, "HELP",, "O Documentos de Frete esta Pendente de Desatualiza��o no Recebimento, � necessario aguardar retorno.", 1, 0)
			Return .F.
		EndIf
		...
		If nOpc == 4
			...
			If GW6->( dbSeek(GW3->GW3_FILFAT + GW3->GW3_EMIFAT + GW3->GW3_SERFAT + GW3->GW3_NRFAT + DToS(GW3->GW3_DTEMFA)) )
				If GW6->GW6_SITAPR $ "3;4"
					...
					Help( ,, "HELP",, "O Documento de Frete nao pode ser alterado, pois esta vinculado a uma Fatura aprovada." + "� necessario que o Documento de Frete seja desvinculado da Fatura para que possa ser efetuada a alteracao.", 1, 0)
					Return .F.
				EndIf
			EndIf
		EndIf
	...
	If nOpc == 5
		...
		If !Empty(GW3->GW3_NRFAT)
			Help( ,, "HELP",, If( cPaisLoc $ "ANG|PTG", "Nao � possivel excluir quando o documento de frete estiver vinculado em uma factura", "Nao � possivel excluir quando o documento de frete estiver vinculado em uma fatura" ), 1, 0)
			Return .F.
		EndIf

		If GW3->GW3_SITREC == "2" .OR.  GW3->GW3_SITREC == "4" .OR.  GW3->GW3_SITREC == "5"
			Help( ,, "HELP",, "Nao � possivel excluir quando a situacao do Recebimento estiver 'Pendente' ou 'Atualizado' ", 1, 0)
			Return .F.
		ElseIf ( GW3->GW3_SITFIS == "2" .OR.  GW3->GW3_SITFIS == "4" .OR.  GW3->GW3_SITFIS == "5" ) .And.  !lCteSubs
			Help( ,, "HELP",, "Nao � possivel excluir quando a situacao do Fiscal estiver 'Pendente' ou 'Atualizado'", 1, 0)
			Return .F.
		EndIf
		...
		If GfeVerCmpo({"GW3_SITMLA"})
			...
			If lAprovMLA .And.  !(GW3->GW3_SITMLA $ "1|6" .Or.  Empty(GW3->GW3_SITMLA))
				Help(,,"HELP",,"Documento de Frete esta integrado ao MLA ou pendente de Integracao, realize a desatualiza��o para esta operacao.",1,0)
				Return .F.
			EndIf
		EndIf
	EndIf
	...
	If cTipo == "1" .OR.  cTipo == "3" .OR.  cTipo == "5"
		...
		While !Eof() .AND.  GW4->GW4_FILIAL + GW4->GW4_EMISDC + GW4->GW4_SERDC + GW4->GW4_NRDC + GW4->GW4_TPDC == xFilial("GW4") + FwFldGet("GW4_EMISDC", nLine) + FwFldGet("GW4_SERDC", nLine) + FwFldGet("GW4_NRDC", nLine) + FwFldGet("GW4_TPDC", nLine)
			...
			If dbSeek(xFilial("GW3")+GW4->GW4_CDESP+GW4->GW4_EMISDF+GW4->GW4_SERDF+GW4->GW4_NRDF+DTOS(GW4->GW4_DTEMIS))
				...
				If  GW3->GW3_CDESP        + GW3->GW3_EMISDF         + GW3->GW3_SERDF        + GW3->GW3_NRDF        + DtoS(GW3->GW3_DTEMIS) <> FwFldGet("GW3_CDESP") + FwFldGet("GW3_EMISDF")  + FwFldGet("GW3_SERDF") + FwFldGet("GW3_NRDF") + DtoS(FwFldGet("GW3_DTEMIS"))
					If GW3->GW3_TPDF == FwFldGet("GW3_TPDF")
						Help( ,, "HELP",, "Nao � possivel vincular o mesmo Documento de Carga em mais de um Documento de Frete do mesmo tipo."+Chr(13)+Chr(10)+Chr(13)+Chr(10)+"- Documento de carga "+alltrim(GW4->GW4_NRDC)+" cd. Emissor "+alltrim(GW4->GW4_EMISDC)+" vinculado ao documento de frete "+alltrim(GW3->GW3_NRDF)+".", 1, 0)
						lRet := .F.
					EndIf
				EndIf
			EndIf
...
User Function G065GW4VPR(oModel)
	...
	If !Empty(FwFldGet("GW3_EMIFAT")) .Or.  !Empty(FwFldGet("GW3_SERFAT")) .Or.  !Empty(FwFldGet("GW3_NRFAT")) .Or.  !Empty(FwFldGet("GW3_DTEMFA")) .Or.  !Empty(FwFldGet("GW3_FILFAT"))
		Help( ,, "HELP",, If( cPaisLoc $ "ANG|PTG", "Nao � possivel vincular/desvincular Documentos de Carga quando o documento de Factura estiver vinculado em uma factura", "Nao � possivel vincular/desvincular Documentos de Carga, quando o documento de Fatura estiver vinculado em uma fatura" ), 1, 0)
		lRet := .F.
	EndIf
	RestArea( aArea )
...
User Function G065GW4VP(oModel)
	...
	If lRet
		...
		If GW1_SIT == "7"
			Help( ,, "HELP",, If( cPaisLoc $ "ANG|PTG", "Nao � possivel vincular um Documento de Carga cancelado", "Nao � possivel vincular um Documento de Carga Cancelado" ), 1, 0)
			lRet := .F.
		EndIf
	EndIf
	...
	If lRet
			...
			While !Eof() .And.  GWU->GWU_FILIAL+GWU->GWU_CDTPDC+GWU->GWU_EMISDC+GWU->GWU_SERDC+GWU->GWU_NRDC == GW1->GW1_FILIAL+GW1->GW1_CDTPDC+GW1->GW1_EMISDC+GW1->GW1_SERDC+GW1->GW1_NRDC
				...
				If GU3->(DbSeek(xFilial("GU3")+FwFldGet("GW3_EMISDF"))) .And.  GV5->(DbSeek(xFilial("GV5")+GW1->GW1_CDTPDC))
					If GWU->GWU_CDTRP == FwFldGet("GW3_EMISDF") .And.  Empty(GWU->GWU_DTENT) .And.  GU3->GU3_ENTOBR == "1" .And.  GV5->GV5_SENTID $ "23"
						If !IsBlind()
							Help( ,, "HELP",, "Nao � possivel vincular um Documento de Carga sem entrega para esse Emissor", 1, 0)
							lRet := .F.
						EndIf
					EndIf
					lTrechoEmi := .T.
		...
		If !lTrechoEmi
			Help( ,, "HELP",, "S� permite incluir Documentos de Cargas, se algum trecho for o Emissor. Verificar se existe calculo vinculado ao Documento de Carga", 1, 0)
			lRet := .F.
		EndIf
	EndIf
	...
	If lRet .And.  oModel:getLine() == 1 .And.  oModel:IsInserted() .And.  Empty(FwFldGet("GW3_CDREM")) .And.  Empty(FwFldGet("GW3_CDDEST"))
		...
		If !Empty(cRem) .And.  !Empty(cDest)
			...
			If Iif(FindFunction("APMsgYesNo"), APMsgYesNo("Deseja utilizar remetente e destinat�rio deste documento de carga para o documento de frete?"+Chr(13)+Chr(10)+"Remetente:"+AllTrim(cRem)+"-"+AllTrim(Posicione("GU3",1,xFilial("GU3")+cRem,"GU3_NMEMIT"))+Chr(13)+Chr(10)+"Destinatario:"+AllTrim(cDest)+"-"+AllTrim(Posicione("GU3",1,xFilial("GU3")+cDest,"GU3_NMEMIT"))+Chr(13)+Chr(10), "Ajuda"), (cMsgYesNo:="MsgYesNo", &cMsgYesNo.("Deseja utilizar remetente e destinat�rio deste documento de carga para o documento de frete?"+Chr(13)+Chr(10)+"Remetente:"+AllTrim(cRem)+"-"+AllTrim(Posicione("GU3",1,xFilial("GU3")+cRem,"GU3_NMEMIT"))+Chr(13)+Chr(10)+"Destinatario:"+AllTrim(cDest)+"-"+AllTrim(Posicione("GU3",1,xFilial("GU3")+cDest,"GU3_NMEMIT"))+Chr(13)+Chr(10), "Ajuda")))
...
User Function UGFE5CO(oModel)
						...
						If dbSeek(xFilial("GU5") + GWD->GWD_CDTIPO)
							...
							If dbSeek(xFilial("GU3") + GWD->GWD_CDTRP)
								If GU3->GU3_ACOCO == "1" .and.  GWD->GWD_CDTRP == FwFldGet("GW3_EMISDF")
									If GU5->GU5_ACAODF == "3"
										lRet := .F.
										cMotBloq := "H� Registro de Ocorrencia para o(s) Documento(s) de Carga do Documento de Frete com acao de bloqueio."
										lMsgOco := .T.
										GFEA115Msg(lMsgOco)
										Exit
			...
			While (cAliasQry)->(!EoF())
				If s_VLCNPJ_2 == "1" .Or.  SubStr(Posicione("GU3", 1, xFilial("GU3") +FwFldGet("GW3_EMISDF"), "GU3->GU3_IDFED"), 1, 8) == SubStr(Posicione("GU3", 1, xFilial("GU3") + (cAliasQry)->GWF_TRANSP, "GU3->GU3_IDFED"), 1, 8)
					...
					If s_AUDINF $ "2" .And.  (cAliasQry)->GWF_ORIGEM == "2"
						lRet := .F.
						cMotBloq := cMotBloq + " H� Frete Combinado"
					EndIf
	...
	If SuperGetMV("MV_DCOUT", .F. ,"N") $ "1S"
		...
		If FwFldGet("GW3_QTVOL") <> iQT_VOL
			lRet := .F.
			cMotBloq := cMotBloq + If( cPaisLoc $ "ANG|PTG", "H� diferenca na Quantidade de Volume", "H� diferenca no Quantidade de Volume" )
		EndIf


		If FwFldGet("GW3_PESOR") <> iPESO_REAL
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Peso Real"
		EndIf


		If FwFldGet("GW3_PESOC") <> iPESO_CUBA
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Peso Cubado"
		EndIf


		If FwFldGet("GW3_VOLUM") <> iVOLUME
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Volume"
		EndIf


		If FwFldGet("GW3_VLCARG") <> iVALOR
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Valor dos Itens"
		EndIf
	EndIf
	...
	If SuperGetMV("MV_DCABE", .F. ,"N") $ "1S"
		...
		If AbaixoTol(FwFldGet("GW3_FRPESO"), iFRET_UNID)
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Frete Unidade"
		EndIf


		If AbaixoTol(FwFldGet("GW3_FRVAL"), iFRET_VAL)
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Frete Valor"
		EndIf


		If AbaixoTol(FwFldGet("GW3_TAXAS"), iTAXAS)
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca nas Taxas"
		EndIf


		If AbaixoTol(FwFldGet("GW3_PEDAG"), iVAL_PEDA)
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Valor do Ped�gio"
		EndIf
	EndIf
	...
	If SuperGetMV("MV_DCTOT", .F. ,"N") $ "1S"
		...
		If AbaixoTol(FwFldGet("GW3_VLDF"), iVAL_FRETE)
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Valor Total do Frete"
		EndIf


		If FwFldGet("GW3_PCIMP") <> iALIQUOTA
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca na Aliquota"
		EndIf


		If AbaixoTol(Round(FwFldGet("GW3_VLIMP"), 2), iVAL_IMPO)
			lRet := .F.
			cMotBloq := cMotBloq + " H� diferenca no Valor do Imposto"
		EndIf
	EndIf
	...
	If ChkObrigat
		...
		If !oModelGW4:IsDeleted()
			...
			If !lVldCalc .and.  !(IsInCallStack("GFEA115") .or.  IsInCallStack("GFEA118"))
				Help( ,, "HELP",, "� necessario que haja um Calculo de Frete para o Documento de Carga que possua o mesmo Tipo do Documento de Frete e Transportador do Calculo igual ao Emissor do Documento de Frete.", 1, 0)
				lRet := .F.
			ElseIf !lVldCalc .and.  (IsInCallStack("GFEA115") .or.  IsInCallStack("GFEA118"))
				Help( ,, "HELP",, "� necessario que haja um Calculo de Frete para o Documento de Carga (Emissor: "+ GW1->GW1_EMISDC + "Tipo: "+ GW1->GW1_CDTPDC +  "Serie; "+ GW1->GW1_SERDC +" e Numero: " + GW1->GW1_NRDC +") que possua o mesmo Tipo do Documento de Frete e Transportador do Calculo igual ao Emissor do Documento de Frete." , 1, 0)
				lRet := .F.
				Return .F.
			EndIf
		EndIf
...
Static Function xGFE65XF(lAutom, nOpc, lExec)
	...
	If !lAutom
		...
		If dData <= dDtUlFe .and.  cERPGFE == "2"
			Help( ,, "Help",, If( cPaisLoc $ "ANG|PTG", "Data de transac��o deve ser posterior a data do ultimo fechamento: ", "Data de transacao deve ser posterior a data do ultimo fechamento: " ) + DToC(dDtUlFe) + " (Parametro MV_DTULFE)", 1, 0 )
			Return .F.
		EndIf
	...
	If cERPGFE == "1"
		...
		If cDTInt == "2"
			...
			Else
				...
				If GW3->GW3_TPDF <> "3"
					Help( ,, "Help",, "Integracao com o fiscal nao executada, pois nao h� Fatura de Frete relacionado a este Documento de Frete para definir a data de transacao. Verificar o parametro 'Data de Transa��o do Documento Fiscal' na aba 'Integra��es Datasul'.", 1, 0 )
					Return .F.
				EndIf
			EndIf
...
Static Function xGFE65XD(cOp)
	...
	If !(lIntGFE == .T.  .And.  cIntGFE2 $ "1S")
		If aRet[1]
			Iif(FindFunction("APMsgInfo"), APMsgInfo("Documento de Frete enviado para desatualiza��o no "+IIf(cOp=="1","Fiscal","Recebimento"),), MsgInfo("Documento de Frete enviado para desatualiza��o no "+IIf(cOp=="1","Fiscal","Recebimento"),))
		Else
			Alert("Documento de Frete nao foi enviada para desatualiza��o no " + IIf(cOp == "1", "Fiscal", "Recebimento") + " por causa do seguinte erro: " + aRet[2][6])
		EndIf
	EndIf
...
Static Function GFEADOCRE(cTpDc, cEmisDc, cSerDc, cNrDc)
	...
	If nCalc == 0
		Help(,,"HELP",, "Nao foram encontrados C�lculos do tipo " + GFEFldInfo("GW3_TPDF", FwFldGet("GW3_TPDF"), 2) + " para o Documento de Carga.", 1, 0)
	ElseIf nDC == 0
		Help(,,"HELP",, "Nao foram encontrados outros Documentos de Carga para vincular ao Documento de Frete.", 1, 0)
	ElseIf nDC == nChange
		Help(,,"HELP",, "Todos os Documentos de Carga relacionados a este j� foram vinculados.", 1, 0)
	EndIf
...
User Function GFE5GFAT(lInterface)
	...
	If lInterface
		...
		If dbSeek(xFilial("GW6") + GW3->GW3_EMISDF + GW3->GW3_SERDF + GW3->GW3_NRDF )
			Help( ,, "Help",, "Fatura j� cadastrada no sistema. A fatura j� existe no sistema portanto este documento de frete devera ser vinculado a uma fatura manualmente.", 1, 0 )
			Return
		EndIf
	...
	If !Empty(GW3->GW3_FILFAT + GW3->GW3_EMIFAT + GW3->GW3_SERFAT + GW3->GW3_NRFAT + DToS(GW3->GW3_DTEMFA))
		...
		If lInterface
			Help( ,, "Help",, "O Documento de Frete j� esta vinculado a uma Fatura.", 1, 0 )
		EndIf
	...
	If GW3->GW3_TPDF == "3" .And.  GW3->GW3_VLDF == 0
		If lInterface
			Help( ,, "HELP",, "Nao � permitido gerar uma fatura para o documento de frete deste tipo com valor zerado.", 1, 0)
		EndIf
...
User Function GFE065VCTE(cChvCte,cEmissor,cSerie,cNumero,dDataEmis)
	...
	If dbSeek(xFilial("GVT")+cCdEsp)
		...
		If GVT->GVT_TPIMP == "1"
			If Len(cChvCte) <> 44
				aRet[2] := "Numero de caracteres informado no campo Chave Ct-e inv�lido."
				Return (aRet)
			EndIf
			...
			If cAAMM <> cAAMMBrw
				aRet[2] := "Ano e m�s da chave difere do documento de frete."
				Return (aRet)
			EndIf
			...
			If cMod <> "57"
				aRet[2] := "Modalidade de transporte inv�lido."
				Return (aRet)
			EndIf
			...
			If !cTpEmis $ "1;4;5;7;8"
				cTpEmisStr := "Codigo inv�lido."

				aRet[2] := "Tipo emissao inv�lida. [" + cTpEmis + "] " + cTpEmisStr

				Return (aRet)
			EndIf

			If !u_GFE065VLDV(cChvCte)
				aRet[2] := "Digito verificador inv�lido."
				Return (aRet)
			EndIf
			...
			If (cAliasTmp)->(!Eof())
				aRet[2] := "J� existe uma chave ct-e igual cadastrada."
		...
		If GU3->(DbSeek(xFilial("GU3")+cEmissor))
			If (GU3->GU3_CTE == "1") .And.  !(u_IsNumeric(cSerie)) .And.  (GVT->GVT_TPIMP <> "2")
				If !lCmpCte .Or.  (lCmpCte .And.  GVT->GVT_CHVCTE <> "3")
					Iif(FindFunction("APMsgInfo"), APMsgInfo("Transportador cadastrado para gerar chave ct-e, serie do documento tem que ser n�merica.",), MsgInfo("Transportador cadastrado para gerar chave ct-e, serie do documento tem que ser n�merica.",))
					Return .F.
				EndIf
			EndIf
		EndIf
...
Static Function DesAtuDocFret(nInt, oModel, cMsg)
	...
	If !lErro
		...
		Else
			...
			lErro 		:= .T.
			lErroDc 	:= .T.
			cMsg 		:= "O documento de Frete nao foi localizado no ERP para desatualiza��o."
		EndIf
		RestArea(aAreaSF1)
		aSize(aAreaSF1,0)
	EndIf
	...
	If IsInCallStack("U_UGFEA065")
		If lErro
			...
			Else
				Iif(FindFunction("APMsgInfo"), APMsgInfo("Documento de Frete nao enviado para desatualiza��o. Verificar motivo no campo 'Mot Rej "+SubStr(cTipInt,1,3)+"'.",), MsgInfo("Documento de Frete nao enviado para desatualiza��o. Verificar motivo no campo 'Mot Rej "+SubStr(cTipInt,1,3)+"'.",))
			EndIf
		Else
...
User Function UGFEACTE(lValCte)
	...
	If SuperGetMv("MV_CHVNFE", .F. , .F. )
		...
		If !lValCte .And.  !aRetVldCte[1] .And.  Len(AllTrim(SubStr(cChvCte,1,Len(cChvCte)-1))) == Len(cChvCte)-1
			Help(,, "HELP",, "Chave Ct-e inv�lida:"+Chr(13)+Chr(10)+aRetVldCte[2], 1, 0)
			Return .F.
		EndIf
...
User Function UGFEPROC(cChvCte)
	...
	If u_GA065TC20()
		...
		If ((!FindFunction("FWLSEnable") .Or.  !FWLSEnable(3100)) .and.  !FwEmpTeste())
			aRet[2] := "Ambiente nao licenciado para o modelo TOTVS Colabora��o 2.0."
			Return aRet
		EndIf

		If nTimeOut = 0
			aRet[2] := "Parametro Tempo de espera para processar retorno da consulta via TOTVS Colabora��o 2.0 nao informado (aba Integra��es ERP)."
			Return aRet
		EndIf

		If Empty(cVerCte) .Or.  cVerCte = " "
			aRet[2] := "Parametro Versao do CTe para TOTVS Colabora��o 2.0 nao informado (aba Integra��es ERP)."
			Return aRet
		EndIf
		...
		while !lEncerraConsulta
			...
			For nCount = 1 to nTimeOut
				If lTela
					oProcess:incRegua1("Aguardando retorno da consulta - " + cValToChar(nTentativa) + "� tentativa...")
					...
					If Len(aDirImpor) = 1
						...
						If File(cNmArqIn)
							...
							If XmlChildEx(oXML, "_RETCONSSITCTE") = NIL
								aRet[2] := "Arquivo de retorno com estrutura inv�lida."
							Else
	...
	Else
		...
		Else
			If Empty(IIf(Empty(GetWscError(3)),GetWscError(1),GetWscError(3)))
				aRet[2] := "Nao foi possivel conectar ao TSS. Verifique as configura��es de conexao. "
			Else
...
Static Function GA065DPar(cParam)
	...
	If Empty(aRet[1])
		aRet[2] := "Nao foi especificado um diretorio para comunica��o com Neogrid no parametro " + cParam + "."
	Else
		If !ExistDir(aRet[1])
			If MakeDir(aRet[1]) <> 0
		...
		If GW4->( dbSeek(xFilial("GW4")+FwFldGet("GW3_EMISDF")+FwFldGet("GW3_CDESP")+FwFldGet("GW3_SERDF")+FwFldGet("GW3_NRDF")+DTOS(FwFldGet("GW3_DTEMIS"))))
			...
			While !Eof() .And.  xFilial("GW4")+GW4->GW4_EMISDF+GW4->GW4_CDESP+GW4->GW4_SERDF+GW4->GW4_NRDF+DTOS(GW4->GW4_DTEMIS) == FwFldGet("GW3_FILIAL")+FwFldGet("GW3_EMISDF")+FwFldGet("GW3_CDESP")+FwFldGet("GW3_SERDF")+FwFldGet("GW3_NRDF")+DTOS(FwFldGet("GW3_DTEMIS"))
				...
				If !lVldCalc .And.  lVerificado
					Help( ,, "HELP",, "� necessario que haja um Calculo de Frete dos Documentos de Carga que possua o mesmo Tipo do Documento de Frete, e Transportador do Calculo igual ao Emissor do Documento de Frete.", 1, 0)
					Return .F.
				EndIf
...
User Function UGFE65DM()
	...
	If lRet .And.  Iif(FindFunction("APMsgNoYes"), APMsgNoYes("Deseja desatualizar o Documento de Frete no MLA?", "Aviso"), (cMsgNoYes:="MsgNoYes", &cMsgNoYes.("Deseja desatualizar o Documento de Frete no MLA?", "Aviso")))
		...
		Iif(FindFunction("APMsgInfo"), APMsgInfo("Documento de Frete enviado para desatualiza��o no MLA.",), MsgInfo("Documento de Frete enviado para desatualiza��o no MLA.",))
	EndIf
...
Static Function UGFEA065PF(cFilp, cNrpf)
	...
	If nCalc == 0
		Help(,,"HELP",, "Nao foram encontrados C�lculos do tipo " + GFEFldInfo("GW3_TPDF", FwFldGet("GW3_TPDF"), 2) + " para o emissor do conhecimento relacionados � pre-fatura.", 1, 0)
	ElseIf nDC == 0
		Help(,,"HELP",, "Nao foram encontrados outros Documentos de Carga para vincular ao Documento de Frete.", 1, 0)
	EndIf
...
User Function UGFEA065CTP(cAcao,lExibeMsg)
	...
	Else
		Do Case
			...
			If GW3->GW3_SITCUS == "2"
				aRet[2] := "Integracao de valores de custo de transporte com SIGATMS j� efetuada para este documento de frete."
			Else
				Do Case
					Case cAcao == "ID"
