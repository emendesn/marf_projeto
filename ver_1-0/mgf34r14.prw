...
±±³Fun‡…o	³ MGF34R14  ³ Autor ³Geronimo Benedito Alves³Data ³ 01.02.18 ³±±
...
User Function MGF34R14()
	...
	Pergunte("MTR480",.T.)				// Abre tela de pergunte para usuario informar os parametros do relatório
	If (mv_par08 - mv_par07 > 180)
		MsgStop("O intervalo entre a data inicial e a data final é maior que o intervalo máximo permitido que é de 180 dias")
		Return Nil
	EndIf
...
Static Function ReportDef(nReg)
	...
	//mv_par09	:= mv_par09	// Esta linha, é apenas p/ na compilacao não aparecer a msg "Local variable mv_par09 never used", que é indevida
	//mv_par11	:= mv_par11	// Esta linha, é apenas p/ na compilacao não aparecer a msg "Local variable mv_par11 never used", que é indevida
	...
	TRCell():New(oSection2,"X5_DESCRI"	,"SX5","Descr."+CRLF+STR0061,/*Picture*/				,20			,/*lPixel*/,{|| (cAliasSB6)->X5_DESCRI  }) //"Descrição Unidade"
	...
	TRCell():New(oSection3,"X5_DESCRI"	,"SX5","Descr."+CRLF+STR0061,/*Picture*/				,20				,/*lPixel*/,{|| (cAliasSB6)->X5_DESCRI  }) //"Descrição Unidade"
...
Static Function ReportPrin(oReport)
	...
	While !oReport:Cancel() .And. !(cAliasSB6)->(Eof()) 
		...
		While !oReport:Cancel() .And. !(cAliasSB6)->(Eof()) ; 	//  .And. cFilAnt == (cAliasSB6)->B6_FILIAL ;	// Ajustei. Troquei xFilial("SB6") por cFilAnt.  A pricípio comentei parte desta linha já que agora o processamento é multi-filial, a posterior,notei não ser necessario
		.And. Iif(nOrdem==1,cQuebra == (cAliasSB6)->(B6_PRODUTO+B6_LOCAL),If(nOrdem==2,cQuebra == (cAliasSB6)->(B6_CLIFOR+B6_LOJA+B6_PRODUTO+B6_TPCF),cQuebra == (cAliasSB6)->(Dtos(B6_EMISSAO)+B6_PRODUTO)))
			...
			ElseIf nOrdem == 3
				...
				oSection2:Cell("nSaldo"):SetValue(If(mv_par15==1,ConvUm((cAliasSB6)->(B6_PRODUTO),nSaldo,0,2),nSaldo))
				oSection2:Cell("nTotNF"):SetValue(round(aSaldo[5],2))
				oSection2:Cell("nTotDev"):SetValue(((cAliasSB6)->B6_QUANT - nSaldo) * nPrUnit)		// Geronimo, verificar se o campo B6_QUANT é realmente da tabela (cAliasSB6), e não da SB6 
				// Custo na Moeda
...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Retorna o nome do cliente ou fornecedor						³±±
±±³			³relatorios que poderao ser agendados pelo usuario.			³±±
±±³			³															³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Retorno	³Nenhum														³±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o	³ CalcTerc ³ Autor ³Geronimo Benedito Alves³ Data ³ 14.05.18 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso		 ³ Especifico para a Marfrig									 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Observacao³ Criada a partir da funcão padrao CalcTerc() do Matxatu.prx ³±±
