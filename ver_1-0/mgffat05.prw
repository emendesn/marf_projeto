	...
	If M->C5_TIPO == "N" .AND. EMPTY(M->C5_MDCONTR) .AND. EMPTY(M->C5_MDNUMED);
		...
		If lRet
			...
			If DbSeek(xFilial("SA1")+xCliente+xLoja)
				...
				If Empty(cRegiao)
					APMsgInfo("O campo regi�o nao esta preenchido no cadastro do cliente !","Atencao")
					lRet	:= .F.
...
Static Function FiltraZA(xTipoPed, xVendedor, xDepto, xCliente, xLoja, xEstado, xRegiao)
	...
	If T05PesTab(@aDadosDA0, xTipoPed, xVendedor, xDepto, xCliente, xLoja, xEstado, xRegiao)
		...
		//--Montagem da Tela
		Define MsDialog oDlgZZY Title "Consulta Tabela de Pre�os" From 0,0 To 280, 500 Of oMainWnd Pixel
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Atribui os valores selecionados �s variaveis staticas para retorno
========================================================================================
*/
Static Function ConfZZY(_nPos, aDadosDA0, _bRet)
	...
	cCodTab := aDadosDA0[_nPos,1]    			//Nao esquecer de alimentar essa variavel quando for f3 pois ela � o retorno
	cDesTab	:= Alltrim(aDadosDA0[_nPos,2])		//Nao esquecer de alimentar essa variavel quando for f3 pois ela � o retorno
