...
If lRet
	...
	If ADL->(dbSeek(xFilial("ADL")+M->ADL_VEND+M->ADL_ENTIDA+M->ADL_CODENT+M->ADL_LOJENT))
		lRet := .F.
		APMsgStop("J� existe registro com esta chave na tabela ADL, Vendedor+Entidade+Cod_Entidade+Loja_Entidade. Verifique!")
	Endif	
Endif
...
If lRet
	If cTipo == "1" // alteracao
		...
		If lRet
			...
			If AO4->(dbSeek(xFilial("AO4")+M->ADL_ENTIDA+Padr(M->ADL_CODENT+M->ADL_LOJENT,TamSX3("AO4_CHVREG")[1])+cUsuarioNew))
				lRet := .F.
				APMsgStop("J� existe registro com esta chave na tabela AO4, Vendedor+Entidade+Cod_Entidade+Loja_Entidade. Verifique!")
			Endif	
		Endif
	Endif
	...
	If lRet
		...
		If AO4->(dbSeek(cChave))
			...
			If (cAliasTrb)->(!Eof())
				lRet := .F.
				APMsgStop("Este registro nao pode ser alterado/excluido pois esta associado a uma ESTRUTURA DE NEG�CIOS")
			Endif
