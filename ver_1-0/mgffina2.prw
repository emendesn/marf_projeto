	...
	While SD2->(!EOF()) .AND. (xFilial("SF2") + SF2->F2_DOC + SF2->F2_SERIE + SF2->F2_CLIENTE + SF2->F2_LOJA) = (SD2->D2_FILIAL + SD2->D2_DOC + SD2->D2_SERIE + SD2->D2_CLIENTE + SD2->D2_LOJA)
		...
		If DbSeek(xFilial("SD2") + SD2->D2_SERIORI + SD2->D2_NFORI)
			...
			.AND. (xFilial("SD2") + SD2->D2_SERIORI + SD2->D2_NFORI) = SE2->E2_FILIAL + SE2->E2_PREFIXO + SE2->E2_NUM
				...
				//Verifica qual o tipo do titulo e se nao o desconto nao � maior que o saldo.
				...
				.AND. (SE2->E2_FORNECE) = (cFORINSS)
					...
					If SE2->E2_VALOR = SE2->E2_SALDO
						...
						ZDS->(MsUnlock())	
						lRecalc := .T. //Houve atualizacao entao � necessario realizar o recalculo.
					Else
						Help( ,, 'MGFFINA2_4',, 'Nao foi possivel incluir o Tipo de valor do titulo ' + SE2->E2_NUM + ' ' + SE2->E2_TIPO + ' - Este j� sofreu baixa', 1, 0)
					EndIf
				...
				.AND. (SE2->E2_FORNECE + SE2->E2_LOJA) = (SD2->D2_CLIENTE + SD2->D2_LOJA)
					...
					If SE2->E2_VALOR = SE2->E2_SALDO
						...
						lRecalc := .T. //Houve atualizacao entao � necessario realizar o recalculo.
					Else
						Help( ,, 'MGFFINA2_3',, 'Nao foi possivel incluir o Tipo de valor do titulo ' + SE2->E2_NUM + ' ' + SE2->E2_TIPO + ' - Este j� sofreu baixa', 1, 0)
					EndIf
				EndIf
...
//Query para armazenar todos os itens da Nota Fiscal excluida. Varre todos os itens da NF, pois � possivel devolver mais de uma Nota Fiscal no mesmo documento.
...
While SD2TMP->(!EOF())	
	...
	If ZDS->(DBSeek(SD2TMP->D2_FILIAL + 'SD2' + SD2TMP->D2_DOC + SD2TMP->D2_SERIE + SD2TMP->D2_CLIENTE + SD2TMP->D2_LOJA + SD2TMP->D2_SERIORI + SD2TMP->D2_NFORI))
		While ZDS->(!EOF()) ;
			...
			//Localiza o titulo origem do tipo de valor para verifica se h� saldo dispon�vel para estorno do tipo de valor.
			DBSelectArea('SE2')
			dbSetOrder(1) // E2_FILIAL + E2_PREFIXO + E2_NUM + E2_PARCELA + E2_TIPO + E2_FORNECE + E2_LOJA
			SE2->(DBGoTop())
