...
Local nPerComis	:=0
  ...
  DEFINE MSDIALOG oDlg3 TITLE "Vendedor x Tabela de Preco" FROM 000, 000  TO 250, 500 COLORS 0, 16777215 PIXEL
	...
	oGet3:bHelp := {||    ShowHelpCpo(    "nPerComis",;
                        {"Percentual de comiss�o para a tabela."},2,;
                        {"Solu��o"},2)}
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Valida se o tipo de pedido esta vinculado � tabela de preco
=====================================================================================
*/
...
If !Empty(xCod)
	...
	If DbSeek(xFilial("SZB")+DA0->DA0_CODTAB+xCod)
		Alert("Filial j� cadastrada para esta tabela de preco!")
		xNom	:= " "
		lRet	:= .F.
	Else
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Valida se o vendedor esta vinculado � tabela de preco
=====================================================================================
*/
...
If !Empty(xCod)
	...
	If DbSeek(xFilial("SZC")+DA0->DA0_CODTAB+xCod)
		Alert("Vendedor j� cadastrado para esta tabela de preco!")
		xDesc		:= " "
		lRet		:= .F.
	Else
		DbSelectArea("SA3")
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Valida se o departamento esta vinculado � tabela de preco
=====================================================================================
*/
...
If !Empty(xCod)
	...
	If DbSeek(xFilial("SZF")+DA0->DA0_CODTAB+xCod)
		Alert("Departamento j� cadastrado para esta tabela de preco!")
		xDesc		:= " "
		lRet		:= .F.
	Else
		DbSelectArea("SZD")
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Valida se o cliente esta vinculado � tabela de preco
=====================================================================================
*/
...
If !Empty(xCod+xLoja)
	...
	If DbSeek(xFilial("SZG")+DA0->DA0_CODTAB+xCod+xLoja)
		Alert("Cliente j� cadastrado para esta tabela de preco!")
		xDesc		:= " "
		lRet		:= .F.
	Else
		DbSelectArea("SA1")
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Valida se o Estado esta vinculado � tabela de preco
=====================================================================================
*/
...
If !Empty(xCod)
	...
	If DbSeek(xFilial("SZH")+DA0->DA0_CODTAB+xCod)
		Alert("Estado j� cadastrado para esta tabela de preco!")
		xDesc		:= " "
		lRet		:= .F.
	Else
		cQuery := " SELECT SX5.X5_CHAVE, SX5.X5_DESCRI "
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Valida se a regiao esta vinculado � tabela de preco
=====================================================================================
*/
...
If !Empty(xCod)
	...
	If DbSeek(xFilial("SZI")+DA0->DA0_CODTAB+xCod)
		Alert("Regiao j� cadastrada para esta tabela de preco!")
		xDesc		:= " "
		lRet		:= .F.
	Else
		DbSelectArea("SZP")
...
If !Empty(xCodTPed)
	...
	If DbSeek(xFilial("SZK")+DA0->DA0_CODTAB+xCodTPed)
		Alert("Tipo de Pedido j� cadastrado para esta tabela de preco!")
		xNomTped	:= " "
		lRet	:= .F.
	Else
		DbSelectArea("SZJ")
	...
	If DbSeek(xFilial("SZM")+DA0->DA0_CODTAB)
		...
		While 	SZL->ZL_FILIAL  == xFilial("SZL")	.AND.;
			...
			If SZL->ZL_VOLUME == xVolume
				Alert("Faixa de Volume j� cadastrado para esta tabela de preco!")
				lRet	:= .F.
				Exit
			Endif
			SZL->(DbSkip())
...
Solicitante.........: Cliente
Uso.................: 
Obs.................: Valida se o % De desconto esta vinculado � tabela de preco
=====================================================================================
*/
	...
	If DbSeek(xFilial("SZM")+DA0->DA0_CODTAB)
		...
		While 	SZM->ZM_FILIAL  == xFilial("SZM")	.AND.;
			...
			If SZM->ZM_QTDE == xQtde
				Alert("Regra j� cadastrado para esta tabela de preco!")
				lRet	:= .F.
				Exit
			Endif
			SZM->(DbSkip())
		...
		If FWFldGet('DA1_PRCVEN') <>  nVlVen
			...
			If Retfield("SB1",1, xFilial("SB1")+ nPrd , "B1_XENVECO") == "1"
				If !ExistCpo("ZC4",Retfield("SB1",1,xFilial("SB1") + nPrd ,"B1_ZLINHA") )
					uRet	:= T04INB1()
					If !uRet
						Help( ,, 'Pend�ncia de cadastro',, 'Campo LINHA do cadastro de produto inv�lido. O preco para esse item nao sera atualizado.' , 1, 0 ,,,,,,; 
...
Else
	ApMsgStop("Arquivo com lay-out inv�lido, devem haver 2 colunas ( Codigo do Vendedor e %Comiss�o ) para cada linha do arquivo.")
Endif
...
If lContinua
	...
	APMsgInfo("Importacao realizada com sucesso.")
Else
	ApMsgStop("Arquivo com lay-out inv�lido, devem haver 2 colunas ( Codigo do Cliente e Loja do Cliente ) para cada linha do arquivo.")
Endif
...
�����������������������������������������������������������������������������
�������������������������������������������������������������������������ͻ��
���Programa  �txtbal    �Autor  � TOTVS              � Data �  Junho/18   ���
�������������������������������������������������������������������������͹��
���Desc.     �Utilizado para Gerar TXT Balan�a Toledo                     ���
���          �                                                            ���
�������������������������������������������������������������������������͹��
���Uso       �                                                            ���
�������������������������������������������������������������������������ͼ��
...
cDirLoc := 	cGetFile('','Selecione o Diret�rio',0,,.F.,GETF_LOCALHARD+ GETF_RETDIRECTORY+GETF_NETWORKDRIVE)
...
Local lRet		:= 	.F.
	...
	DEFINE MSDIALOG oDlgMain TITLE "Atualizar cadastro de produto" FROM 000, 000  TO 190, 500 COLORS 0, 16777215 PIXEL
		...
		nUltLin += 18
		@ nUltLin		,nCol SAY "Campo LINHA do cadastro de produto inv�lido." SIZE 176, 07 OF oDlgMain  PIXEL 
